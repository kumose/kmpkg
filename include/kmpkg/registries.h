...
namespace kmpkg
{
    inline constexpr StringLiteral builtin_registry_git_url = "https://github.com/kumose/kmpkg";
    ...
    struct LockFile
    {
        ...
        struct Entry
        {
            ...
            ExpectedL<Unit> ensure_up_to_date(const KmpkgPaths& paths) const;
        };

        ExpectedL<Entry> get_or_fetch(const KmpkgPaths& paths, StringView repo, StringView reference);
    ...
    ExpectedL<std::map<std::string, Version, std::less<>>> get_builtin_baseline(const KmpkgPaths& paths);
