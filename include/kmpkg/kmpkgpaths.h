...
#include <kmpkg/base/fwd/git.h>
#include <kmpkg/base/fwd/system.h>
#include <kmpkg/base/fwd/system.process.h>
...
namespace kmpkg
{
    ...
    struct KmpkgPaths
    {
        KmpkgPaths(const Filesystem& filesystem, const KmpkgCmdArguments& args, const BundleSettings& bundle);
        KmpkgPaths(const KmpkgPaths&) = delete;
        KmpkgPaths& operator=(const KmpkgPaths&) = delete;
        ~KmpkgPaths();
    ...
    private:
        const std::unique_ptr<KmpkgPathsImpl> m_pimpl;
    ...
    public:
        ...
        // Git manipulation in the kmpkg directory
