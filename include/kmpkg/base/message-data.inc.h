...
DECLARE_MESSAGE(AddingCompletionEntry, (msg::path), "", "Adding kmpkg completion entry to {path}.")
...
DECLARE_MESSAGE(AddPortSucceeded, (), "", "Succeeded in adding ports to kmpkg.json file.")
...
DECLARE_MESSAGE(AddVersionArtifactsOnly,
                (),
                "'--version', and 'kmpkg add port' are command lines that must not be localized",
                "--version is artifacts only and can't be used with kmpkg add port")
...
DECLARE_MESSAGE(AmbiguousConfig,
                (msg::json_field),
                "",
                "Ambiguous kmpkg configuration provided by both manifest and configuration file. Choose one by "
...
DECLARE_MESSAGE(AnObjectContainingKmpkgArtifactsMetadata,
                (),
                "'kmpkg-artifacts' is the name of the product feature and should not be localized",
                "an object containing kmpkg-artifacts metadata")
...
DECLARE_MESSAGE(AppliedUserIntegration, (), "", "Applied user-wide integration for this kmpkg root.")
...
DECLARE_MESSAGE(
    ...
    (),
    "",
    "one or more ports requested to be installed were not present in the action plan. (Probably a kmpkg bug)")
DECLARE_MESSAGE(ArtifactsBootstrapFailed, (), "", "kmpkg-artifacts is not installed and could not be bootstrapped.")
...
DECLARE_MESSAGE(AssetCacheMissBlockOrigin,
                (msg::url),
                "x-block-origin is a kmpkg term. Do not translate",
...
DECLARE_MESSAGE(AssetSourcesArg, (), "", "Asset caching sources. See 'kmpkg help assetcaching'")
...
DECLARE_MESSAGE(ToolDataFileSchemaVersionNotSupported,
                (msg::version),
                "",
                "document schema version {version} is not supported by this version of kmpkg")
DECLARE_MESSAGE(AttemptingToSetBuiltInBaseline,
                (),
                "",
                "attempting to set builtin-baseline in kmpkg.json while overriding the default-registry in "
                "kmpkg-configuration.json.\nthe default-registry from kmpkg-configuration.json will be used.")
...
DECLARE_MESSAGE(AKmpkgRepositoryCommit, (), "", "a kmpkg repository commit")
...
DECLARE_MESSAGE(BaselineConflict,
                (),
                "",
                "Specifying kmpkg-configuration.default-registry in a manifest file conflicts with built-in "
...
DECLARE_MESSAGE(BinarySourcesArg,
                (),
                "'kmpkg help binarycaching' is a command line and should not be localized",
                "Binary caching sources. See 'kmpkg help binarycaching'")
...
DECLARE_MESSAGE(BuildTroubleshootingMessage1,
                ...
                "Please ensure you're using the latest port files with `git pull` and `kmpkg "
...
DECLARE_MESSAGE(
    ...
    "before version information about kmpkg itself.",
...
DECLARE_MESSAGE(
    ...
    "build system. To suppress this message, add set(KMPKG_POLICY_SKIP_ARCHITECTURE_CHECK enabled)")
DECLARE_MESSAGE(ChecksFailedCheck, (), "", "kmpkg has crashed; no additional details are available.")
...
DECLARE_MESSAGE(CmdAddExample1, (), "", "kmpkg add port <port name>")
DECLARE_MESSAGE(CmdAddExample2, (), "", "kmpkg add artifact <artifact name>")
...
DECLARE_MESSAGE(CmdAddVersionOptSkipFormatChk, (), "", "Skips the formatting check of kmpkg.json files")
...
DECLARE_MESSAGE(CmdBootstrapStandaloneSynopsis, (), "", "Bootstraps a kmpkg root from only a kmpkg binary")
DECLARE_MESSAGE(CmdBuildExternalExample1,
                (),
                "This is a command line, only the <>s part should be localized",
                "kmpkg build-external <port name> <source path>")
DECLARE_MESSAGE(CmdBuildExternalExample2,
                ...
                "kmpkg build-external zlib2 C:\\path\\to\\dir\\with\\kmpkg.json")
DECLARE_MESSAGE(CmdBuildExample1,
                (),
                "This is a command line, only the <>s part should be localized",
                "kmpkg build <port spec>")
...
DECLARE_MESSAGE(CmdCheckSupportExample1,
                (),
                "This is a command line, only the <>s part should be localized",
                "kmpkg x-check-support <port name>")
...
DECLARE_MESSAGE(CmdDownloadExample1,
                ...
                "kmpkg x-download <filepath> <sha512> --url=https://...")
DECLARE_MESSAGE(CmdDownloadExample2,
                (),
                "This is a command line, only the <filepath> part should be localized",
                "kmpkg x-download <filepath> --sha512=<sha512> --url=https://...")
DECLARE_MESSAGE(CmdDownloadExample3,
                (),
                "This is a command line, only the <filepath> part should be localized",
                "kmpkg x-download <filepath> --skip-sha512 --url=https://...")
...
DECLARE_MESSAGE(CommandEnvExample2,
                ...
                "kmpkg env \"ninja -C <path>\" --triplet x64-windows")
...
DECLARE_MESSAGE(CmdExportExample1,
                ...
                "kmpkg export <port names> [--nuget] [--output-dir=out_dir]")
...
DECLARE_MESSAGE(CmdFindExample1,
                (),
                "This is a command line, only the <>s part should be localized",
                "kmpkg find port <port name>")
DECLARE_MESSAGE(CmdFindExample2,
                (),
                "This is a command line, only the <>s part should be localized",
                "kmpkg find artifact <artifact name>")
...
DECLARE_MESSAGE(CmdFormatFeatureBaselineExample,
                (),
                "This is a command line, only the <kmpkg.json path>s part should be localized",
                "kmpkg format-feature-baseline <ci.feature.baseline.txt path>")
DECLARE_MESSAGE(CmdFormatManifestExample1,
                (),
                "This is a command line, only the <kmpkg.json path>s part should be localized",
                "kmpkg format-manifest <kmpkg.json path>")
...
DECLARE_MESSAGE(CmdFormatManifestSynopsis, (), "", "Prettyfies kmpkg.json")
DECLARE_MESSAGE(CmdGenerateMSBuildPropsExample1,
                (),
                "This is a command line, only the <path> part should be localized",
                "kmpkg generate-msbuild-props --msbuild-props <path>")
DECLARE_MESSAGE(CmdGenerateMSBuildPropsExample2,
                (),
                "This is a command line, only the word 'out' should be localized",
                "kmpkg generate-msbuild-props --msbuild-props out.props")
DECLARE_MESSAGE(
...
DECLARE_MESSAGE(CmdInstallExample1,
                ...
                "kmpkg install <port name> <port name>...")
DECLARE_MESSAGE(CmdIntegrateSynopsis, (), "", "Integrates kmpkg with machines, projects, or shells")
...
DECLARE_MESSAGE(CmdNewExample1,
                ...
                "kmpkg new --name=example --version=1.0")
DECLARE_MESSAGE(CmdNewOptApplication, (), "", "Creates an application manifest (don't require name or version)")
DECLARE_MESSAGE(CmdNewOptSingleFile, (), "", "Embeds kmpkg-configuration.json into kmpkg.json")
...
DECLARE_MESSAGE(CmdSetInstalledExample1,
                (),
                "This is a command line, only the part <package name> should be localized.",
                "kmpkg x-set-installed <package name> <package name>...")
...
DECLARE_MESSAGE(CmdSetInstalledOptWritePkgConfig,
                ...
                "See `kmpkg help binarycaching` for more information")
...
DECLARE_MESSAGE(CmdUpdateBaselineOptInitial,
                (),
                "",
                "Adds a `builtin-baseline` to a kmpkg.json that doesn't already have it")
...
DECLARE_MESSAGE(CmdUpdateRegistryExample3,
                ...
                "kmpkg x-update-registry <artifact registry name>")
...
DECLARE_MESSAGE(CmdZExtractExample1,
                (),
                "This is a command line, only the parts in <>s should be localized",
                "kmpkg z-extract <archive path> <output directory>")
DECLARE_MESSAGE(CmdZExtractExample2,
                ...
                "kmpkg z-extract source.zip source_dir --strip 2")
...
DECLARE_MESSAGE(ConflictingEmbeddedConfiguration,
                (),
                "",
                "only one of {{\"configuration\", \"kmpkg-configuration\"}} may be used")
...
DECLARE_MESSAGE(CopyrightIsDir,
                ...
                "file. Consider combining separate copyright files into one using kmpkg_install_copyright. To suppress "
                "this message, add set(KMPKG_POLICY_SKIP_COPYRIGHT_CHECK enabled)")
DECLARE_MESSAGE(CorruptedDatabase,
                (),
                "",
                "kmpkg's installation database corrupted. This is either a bug in kmpkg or something else has modified "
                ...
                "consistently, please file a bug at https://github.com/kumose/kmpkg .")
...
DECLARE_MESSAGE(DefaultBinaryCachePlatformCacheRequiresAbsolutePath,
                (msg::path),
                "",
                "Environment variable KMPKG_DEFAULT_BINARY_CACHE must be a directory (was: {path})")
DECLARE_MESSAGE(DefaultBinaryCacheRequiresAbsolutePath,
                (msg::path),
                "",
                "Environment variable KMPKG_DEFAULT_BINARY_CACHE must be absolute (was: {path})")
...
DECLARE_MESSAGE(DocumentedFieldsSuggestUpdate,
                (),
                "",
                "If these are documented fields that should be recognized try updating the kmpkg tool.")
...
DECLARE_MESSAGE(
    ...
    "2. If you are using Windows, kmpkg will automatically use your Windows IE Proxy Settings set by your "
    "proxy software. See: https://github.com/kumose/kmpkg-tool/pull/77\n"
    "The value set by your proxy might be wrong, or have same `https://` prefix issue.\n"
    "3. Your proxy's remote server is out of service.\n"
    "If you believe this is not a temporary download server failure and kmpkg needs to be changed to download this "
    "file from a different location, please submit an issue to https://github.com/kumose/kmpkg/issues")
...
DECLARE_MESSAGE(DownloadingKmpkgStandaloneBundle, (msg::version), "", "Downloading standalone bundle {version}.")
DECLARE_MESSAGE(DownloadingKmpkgStandaloneBundleLatest, (), "", "Downloading latest standalone bundle.")
...
DECLARE_MESSAGE(EmailKmpkgTeam, (msg::url), "", "Send an email to {url} with any feedback.")
...
DECLARE_MESSAGE(ErrorIndividualPackagesUnsupported,
                (),
                "",
                "In manifest mode, `kmpkg install` does not support individual package arguments.\nTo install "
                "additional packages, edit kmpkg.json and then run `kmpkg install` without any package arguments.")
...
DECLARE_MESSAGE(
    ErrorMissingKmpkgRoot,
    (),
    "",
    "Could not detect kmpkg-root. If you are trying to use a copy of kmpkg that you've built, you must "
    "define the KMPKG_ROOT environment variable to point to a cloned copy of https://github.com/kumose/kmpkg.")
...
DECLARE_MESSAGE(ErrorRequireBaseline,
                (),
                "",
                "this kmpkg instance requires a manifest with a specified baseline in order to "
                "interact with ports. Please add 'builtin-baseline' to the manifest or add a "
                "'kmpkg-configuration.json' that redefines the default registry.")
DECLARE_MESSAGE(ErrorRequirePackagesList,
                (),
                "",
                "`kmpkg install` requires a list of packages to install in classic mode.")
...
DECLARE_MESSAGE(ErrorUnableToDetectCompilerInfo,
                ...
                "kmpkg was unable to detect the active compiler's information. See above for the CMake failure output.")
DECLARE_MESSAGE(ErrorVcvarsUnsupported,
                (msg::triplet),
                "",
                "in triplet {triplet}: Use of Visual Studio's Developer Prompt is unsupported "
                "on non-Windows hosts.\nDefine 'KMPKG_CMAKE_SYSTEM_NAME' or "
                "'KMPKG_CHAINLOAD_TOOLCHAIN_FILE' in the triplet file.")
...
DECLARE_MESSAGE(FilesContainAbsolutePath1,
                ...
                "message, add set(KMPKG_POLICY_SKIP_ABSOLUTE_PATHS_CHECK enabled)")
DECLARE_MESSAGE(FilesContainAbsolutePath2, (), "", "absolute paths found here")
DECLARE_MESSAGE(FilesContainAbsolutePathPkgconfigNote,
                (),
                "",
                "Adding a call to `kmpkg_fixup_pkgconfig()` may fix absolute paths in .pc files")
DECLARE_MESSAGE(FindVersionArtifactsOnly,
                (),
                "'--version', 'kmpkg search', and 'kmpkg find port' are command lines that must not be localized",
                "--version can't be used with kmpkg search or kmpkg find port")
...
DECLARE_MESSAGE(FindCommandFirstArg,
                (),
                "'find', 'artifact', and 'port' are kmpkg specific terms and should not be translated.",
                "The first argument to 'find' must be 'artifact' or 'port' .")
DECLARE_MESSAGE(FishCompletion, (msg::path), "", "kmpkg fish completion is already added at \"{path}\".")
...
DECLARE_MESSAGE(FollowingPackagesMissingControl,
                (),
                "",
                "The following packages do not have a valid CONTROL or kmpkg.json:")
...
DECLARE_MESSAGE(
    ...
    (),
    "",
    "Environment variable KMPKG_FORCE_SYSTEM_BINARIES must be set on arm, s390x, ppc64le and riscv platforms.")
...
DECLARE_MESSAGE(ForMoreHelp,
                (),
                "Printed before a suggestion for the user to run `kmpkg help <topic>`",
                "For More Help")
...
DECLARE_MESSAGE(GhaBinaryCacheDeprecated,
                (msg::url),
                "The term 'x-gha' is a kmpkg configuration option",
...
DECLARE_MESSAGE(
    ...
    "\n"
    "kmpkg can use mirrors to cache downloaded assets, ensuring continued operation even if the "
    "original source changes or disappears.\n"
    "\n"
    "Asset caching can be configured either by setting the environment variable X_KMPKG_ASSET_SOURCES "
...
DECLARE_MESSAGE(
    ...
    "kmpkg can cache compiled packages to accelerate restoration on a single machine or across the network. By "
    "default, kmpkg will save builds to a local machine cache. This can be disabled by passing "
    ...
    "command line or setting the `KMPKG_BINARY_SOURCES` environment variable to a set of sources (Example: "
...
DECLARE_MESSAGE(HelpBinaryCachingNuGetFooter,
                ...
                "must not be localized. Printed before an example XML snippet kmpkg generates when the indicated "
                "environment variables are set.",
                "NuGet's cache is not used by default. To use it for every NuGet-based source, set the environment "
                "variable \"KMPKG_USE_NUGET_CACHE\" to \"true\" (case-insensitive) or \"1\".\n"
...
DECLARE_MESSAGE(HelpBuiltinBase,
                (),
                "",
                "The baseline references a commit within the kmpkg repository that establishes a minimum version on "
...
DECLARE_MESSAGE(HelpExampleCommand, (), "", "For more help (including examples) see https://learn.microsoft.com/kmpkg")
...
DECLARE_MESSAGE(
    ...
    (),
    "",
    "When used as the top-level manifest (such as when running `kmpkg install` in the directory), overrides "
...
DECLARE_MESSAGE(HelpResponseFileCommand,
                (),
                "Describing what @response_file does on kmpkg's command line",
...
DECLARE_MESSAGE(HelpTxtOptWritePkgConfig,
                (),
                "'kmpkg help binarycaching' is a command line and should not be localized.",
                "Writes a NuGet packages.config-formatted file for use with external binary caching. See `kmpkg help "
...
DECLARE_MESSAGE(
    IgnoringKmpkgRootEnvironment,
    (msg::path, msg::actual, msg::value),
    "{actual} is the path we actually used, {value} is the path to kmpkg's binary",
    "The kmpkg {value} is using detected kmpkg root {actual} and ignoring mismatched KMPKG_ROOT environment "
    "value {path}. To suppress this message, unset the environment variable or use the --kmpkg-root command line "
    "switch.")
...
DECLARE_MESSAGE(InternalCICommand,
                (),
                "",
                "kmpkg ci is an internal command which will change incompatibly or be removed at any time.")
DECLARE_MESSAGE(
    InternalErrorMessageContact,
    (),
    "",
    "Please open an issue at "
    "https://github.com/kumose/kmpkg/issues/new?template=other-type-of-bug-report.md&labels=category:kmpkg-bug "
...
DECLARE_MESSAGE(InvalidCommentStyle,
                (),
                "",
                "kmpkg does not support c-style comments, however most objects allow $-prefixed fields to be used as "
                "comments.")
...
DECLARE_MESSAGE(
    ...
    "'{value}' is the linkage type kmpkg would did not understand. (Correct values would be static ofr dynamic)",
...
DECLARE_MESSAGE(InvalidValueHashAdditionalFiles,
                (msg::path),
                "",
                "Variable KMPKG_HASH_ADDITIONAL_FILES contains invalid file path: '{path}'. The value must be "
                "an absolute path to an existent file.")
DECLARE_MESSAGE(InvalidValuePostPortfileIncludes,
                (msg::path),
                "",
                "Variable KMPKG_POST_PORTFILE_INCLUDES contains invalid file path: '{path}'. The value must be "
...
DECLARE_MESSAGE(MismatchedBinParagraphs,
                ...
                "issue at https://github.com/kumose/kmpkg with the following output:")
...
DECLARE_MESSAGE(MismatchedManifestAfterReserialize,
                ...
                "https://github.com/kumose/kmpkg, with the following output:")
...
DECLARE_MESSAGE(NewConfigurationAlreadyExists,
                (msg::path),
                "",
                "Creating a manifest would overwrite a kmpkg-configuration.json at {path}.")
...
DECLARE_MESSAGE(PackageLicenseSpdxThisInstall,
                (),
                "",
                "Packages installed in this kmpkg installation declare the following licenses:")
...
DECLARE_MESSAGE(PathMustBeAbsolute,
                (msg::path),
                "",
                "Value of environment variable X_KMPKG_REGISTRIES_CACHE is not absolute: {path}")
...
DECLARE_MESSAGE(PortBugBinDirExists,
                ...
                "add set(KMPKG_POLICY_DLLS_IN_STATIC_LIBRARY enabled)")
DECLARE_MESSAGE(PortBugDebugShareDir,
                ...
                "suppress this message, add set(KMPKG_POLICY_ALLOW_DEBUG_SHARE enabled)")
DECLARE_MESSAGE(
    ...
    "suppress this message, add set(KMPKG_POLICY_SKIP_APPCONTAINER_CHECK enabled)")
DECLARE_MESSAGE(
    PortBugDllInLibDir,
    (),
    "",
...
DECLARE_MESSAGE(PortBugDuplicateIncludeFiles,
                ...
                "set(KMPKG_POLICY_ALLOW_DEBUG_INCLUDE enabled)")
DECLARE_MESSAGE(
...
DECLARE_MESSAGE(PortBugFoundDllInStaticBuild,
                ...
                "message, add set(KMPKG_POLICY_DLLS_IN_STATIC_LIBRARY enabled)")
DECLARE_MESSAGE(PortBugFoundEmptyDirectories,
                ...
                "suppress this message, add set(KMPKG_POLICY_ALLOW_EMPTY_FOLDERS enabled)")
DECLARE_MESSAGE(
    ...
    "`kmpkg_copy_tools`. To suppress this message, add set(KMPKG_POLICY_ALLOW_EXES_IN_BIN enabled)")
...
DECLARE_MESSAGE(
    ...
    "D:\\kmpkg\\ports\\wrong-crt\\portfile.cmake: warning: binaries built by this port link with C RunTimes (\"CRTs\") "
    "inconsistent with those requested by the triplet and deployment structure. If the triplet is intended to only use "
    "the release CRT, you should add set(KMPKG_POLICY_ONLY_RELEASE_CRT enabled) to the triplet .cmake file. To "
    "suppress this check entirely, add set(KMPKG_POLICY_SKIP_CRT_LINKAGE_CHECK enabled) to the triplet .cmake if this "
    "is triplet-wide, or to portfile.cmake if this is specific to the port. You can inspect the binaries with: "
    "dumpbin.exe /directives mylibfile.lib\n"
    "D:\\kmpkg\\packages\\wrong-crt_x86-windows-static: note: the binaries are relative to ${{CURRENT_PACKAGES_DIR}} "
    "here\n"
    ...
    "add set(KMPKG_POLICY_ONLY_RELEASE_CRT enabled) to the triplet .cmake file. To suppress this check "
    "entirely, add set(KMPKG_POLICY_SKIP_CRT_LINKAGE_CHECK enabled) to the triplet .cmake if this is "
...
DECLARE_MESSAGE(
    ...
    "`dumpbin.exe /dependents mylibfile.dll`. To suppress this message, add set(KMPKG_POLICY_ALLOW_KERNEL32_FROM_XBOX "
    "enabled)")
DECLARE_MESSAGE(
    ...
    "function kmpkg_cmake_config_fixup() from the port kmpkg-cmake-config. To suppress this message, add "
    "set(KMPKG_POLICY_SKIP_LIB_CMAKE_MERGE_CHECK enabled)")
DECLARE_MESSAGE(PortBugMismatchingNumberOfBinaries,
                ...
                "components for this triplet, the triplet should have set(KMPKG_BUILD_TYPE release) added to its "
                ".cmake file. To suppress this message, add set(KMPKG_POLICY_MISMATCHED_NUMBER_OF_BINARIES enabled)")
DECLARE_MESSAGE(PortBugMisplacedCMakeFiles,
                ...
                "set(KMPKG_POLICY_SKIP_MISPLACED_CMAKE_FILES_CHECK enabled)")
DECLARE_MESSAGE(PortBugMisplacedFiles,
                ...
                "set(KMPKG_POLICY_SKIP_MISPLACED_REGULAR_FILES_CHECK enabled)")
DECLARE_MESSAGE(
    ...
    "message, add set(KMPKG_POLICY_SKIP_PKGCONFIG_CHECK enabled)")
DECLARE_MESSAGE(PortBugMissingDebugBinaries, (), "", "Debug binaries were not found.")
DECLARE_MESSAGE(
    PortBugMissingCMakeHelperPortFile,
    (),
    "",
    "The ${{CURRENT_PACKAGES_DIR}}/share/${{PORT}}/kmpkg-port-config.cmake file does not exist. This file must exist "
    "for CMake helper ports. To suppress this message, remove set(KMPKG_POLICY_CMAKE_HELPER_PORT enabled)")
...
DECLARE_MESSAGE(PortBugMissingIncludeDir,
                ...
                "are not correctly installed. If this is a CMake helper port, add set(KMPKG_POLICY_CMAKE_HELPER_PORT "
                "enabled). If this is not a CMake helper port but this is otherwise intentional, add "
                "set(KMPKG_POLICY_EMPTY_INCLUDE_FOLDER enabled) to suppress this message.")
DECLARE_MESSAGE(
    ...
    "a call to kmpkg_install_copyright. To suppress this message, add set(KMPKG_POLICY_SKIP_COPYRIGHT_CHECK enabled)")
DECLARE_MESSAGE(PortBugMissingLicenseFixIt,
                ...
                "kmpkg_install_copyright(FILE_LIST ${{SOURCE_PATH}}/COPYING ${{SOURCE_PATH}}/LICENSE.txt)",
...
DECLARE_MESSAGE(
    ...
    "using a custom triplet targeting an old CRT, add set(KMPKG_POLICY_ALLOW_OBSOLETE_MSVCRT enabled) to the triplet's "
    ".cmake file. To suppress this message for this port, add set(KMPKG_POLICY_ALLOW_OBSOLETE_MSVCRT enabled)")
DECLARE_MESSAGE(
    PortBugRemoveBinDir,
    (),
    "",
...
DECLARE_MESSAGE(
    ...
    "warning can be suppressed by adding set(KMPKG_POLICY_ALLOW_RESTRICTED_HEADERS enabled)")
...
DECLARE_MESSAGE(PortBugSetDllsWithoutExports,
                ...
                "script. If this is intended, add set(KMPKG_POLICY_DLLS_WITHOUT_EXPORTS enabled)")
...
DECLARE_MESSAGE(PortMissingManifest2,
                (msg::package_name),
                "",
                "{package_name} port manifest missing (no kmpkg.json or CONTROL file)")
...
DECLARE_MESSAGE(RemoveDependencies,
                (),
                "",
                "To remove dependencies in manifest mode, edit your manifest (kmpkg.json) and run 'install'.")
...
DECLARE_MESSAGE(ShallowRepositoryDetected,
                (),
                "",
                "kmpkg was cloned as a shallow repository. Try again with a full kmpkg clone.")
...
DECLARE_MESSAGE(SourceFieldPortNameMismatch,
                ...
                "'CONTROL', 'kmpkg.json', and 'name' references are locale-invariant.",
                "The 'Source' field inside the CONTROL file, or \"name\" field inside the kmpkg.json "
...
DECLARE_MESSAGE(SpecifyHostArch,
                (msg::env_var),
                "'kmpkg help triplet' is a command line that should not be localized",
                "Host triplet. See 'kmpkg help triplet' (default: {env_var})")
DECLARE_MESSAGE(SpecifyTargetArch,
                (msg::env_var),
                "'kmpkg help triplet' is a command line that should not be localized",
                "Target triplet. See 'kmpkg help triplet' (default: {env_var})")
...
DECLARE_MESSAGE(UndeterminedToolChainForTriplet,
                ...
                "you mean to use KMPKG_CHAINLOAD_TOOLCHAIN_FILE?")
...
DECLARE_MESSAGE(UnknownParameterForIntegrate,
                (msg::value),
                "'{value}' is a user-supplied command line option. For example, given kmpkg integrate frobinate, "
...
DECLARE_MESSAGE(UnknownSettingForBuildType,
                (msg::option),
                "",
                "Unknown setting for KMPKG_BUILD_TYPE {option}. Valid settings are '', 'debug', and 'release'.")
DECLARE_MESSAGE(UnknownTool, (), "", "kmpkg does not have a definition of this tool for this platform.")
DECLARE_MESSAGE(UnknownTopic,
                (msg::value),
                "{value} the value a user passed to `kmpkg help` that we don't understand",
                "unknown topic {value}")
...
DECLARE_MESSAGE(UnsupportedFeatureSupportsExpression,
                ...
                "build {package_name} anyway, rerun kmpkg with `--allow-unsupported`.")
...
DECLARE_MESSAGE(UnsupportedSystemName,
                (msg::system_name),
                "",
                "Could not map KMPKG_CMAKE_SYSTEM_NAME '{system_name}' to a vcvarsall platform. "
...
DECLARE_MESSAGE(UnsupportedUpdateCMD,
                (),
                "",
                "the update command does not currently support manifest mode. Instead, modify your kmpkg.json and "
                "run install.")
...
DECLARE_MESSAGE(UpdateBaselineLocalGitError,
                (msg::path),
                "",
                "git failed to parse HEAD for the local kmpkg registry at \"{path}\"")
DECLARE_MESSAGE(UpdateBaselineNoConfiguration,
                (),
                "",
                "neither `kmpkg.json` nor `kmpkg-configuration.json` exist to update.")
...
DECLARE_MESSAGE(
    UpgradeInManifest,
    (),
    "'kmpkg x-update-baseline' and 'kmpkg install' are command lines and should not be localized.",
    "Upgrade upgrades a classic mode installation and thus does not support manifest mode. Consider updating your "
    "dependencies by updating your baseline to a current value with kmpkg x-update-baseline and running kmpkg install.")
DECLARE_MESSAGE(
...
DECLARE_MESSAGE(KmpkgCeIsExperimental,
                (),
                "The name of the feature is 'kmpkg-artifacts' and should be singular despite ending in s",
                "kmpkg-artifacts is experimental and may change at any time.")
DECLARE_MESSAGE(
    KmpkgCompletion,
    (msg::value, msg::path),
    "'{value}' is the subject for completion. i.e. bash, zsh, etc.",
    "kmpkg {value} completion is already imported to your \"{path}\" file.\nThe following entries were found:")
DECLARE_MESSAGE(KmpkgDisallowedClassicMode,
                (),
                "",
                "Could not locate a manifest (kmpkg.json) above the current working "
                "directory.\nThis kmpkg distribution does not have a classic mode instance.")
DECLARE_MESSAGE(
    KmpkgHasCrashed,
    (),
    "Printed at the start of a crash report.",
    "kmpkg has crashed. Please create an issue at https://github.com/kumose/kmpkg containing a brief summary of "
    "what you were trying to do and the following information.")
DECLARE_MESSAGE(KmpkgInvalidCommand, (msg::command_name), "", "invalid command: {command_name}")
DECLARE_MESSAGE(KmpkgUsage,
                (),
                "This is describing a command line, everything should be localized except 'kmpkg'; symbols like <>s, "
...
DECLARE_MESSAGE(KmpkgInVsPrompt,
                (msg::value, msg::triplet),
                "'{value}' is a VS prompt",
                "kmpkg appears to be in a Visual Studio prompt targeting {value} but installing for {triplet}. "
                "Consider using --triplet {value}-windows or --triplet {value}-uwp.")
DECLARE_MESSAGE(KmpkgRegistriesCacheIsNotDirectory,
                (msg::path),
                "",
                "Value of environment variable X_KMPKG_REGISTRIES_CACHE is not a directory: {path}")
DECLARE_MESSAGE(KmpkgRootRequired, (), "", "Setting KMPKG_ROOT is required for standalone bootstrap.")
DECLARE_MESSAGE(KmpkgRootsDir, (msg::env_var), "", "The kmpkg root directory (default: {env_var})")
DECLARE_MESSAGE(KmpkgSendMetricsButDisabled, (), "", "passed --sendmetrics, but metrics are disabled.")
...
DECLARE_MESSAGE(VersionCommandHeader,
                (msg::version),
                "",
                "kmpkg package management program version {version}\n\nSee LICENSE.txt for license information.")
DECLARE_MESSAGE(VersionConstraintNotInDatabase1,
                ...
                "version database. All versions must exist in the version database to be interpreted by kmpkg.")
...
DECLARE_MESSAGE(VersionIncomparable4, (msg::url), "", "See `kmpkg help versioning` or {url} for more information.")
...
DECLARE_MESSAGE(VersionRejectedDueToBaselineMissing,
                ...
                "`kmpkg help versioning` for more information.")
