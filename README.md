...
[中文总览](https://learn.microsoft.com/zh-cn/kmpkg/get_started/overview)
[Español](https://learn.microsoft.com/es/kmpkg/get_started/overview)
[한국어](https://learn.microsoft.com/ko-kr/kmpkg/get_started/overview)
[Français](https://learn.microsoft.com/fr/kmpkg/get_started/overview)

Kmpkg helps you manage C and C++ libraries on Windows, Linux and MacOS.
This tool and ecosystem are constantly evolving, and we always appreciate contributions!

Please see the main repository https://github.com/microsoft/kmpkg for all feature discussion, issue
tracking, and edits to which libraries are available.

# Kmpkg-tool: Overview

This repository contains the contents formerly at https://github.com/microsoft/kmpkg in the
"toolsrc" tree, and build support.

# kmpkg-artifacts

kmpkg-artifacts are currently in 'preview' -- there will most certainly be changes between now
...
While the usage of `kmpkg-artifacts` is the same on all platforms, the installation/loading/removal is slightly different depending on the platform you're using.

`kmpkg-artifacts` don't persist any changes to the environment, nor do they automatically add themselves to the start-up environment. If you wish to make it load in a window, you can just execute the script. Manually adding that in your profile will load it in every new window.
...
|-----------------|-----------------------------------------------------|-----------------------|---------------------------------|
| **PowerShell/Pwsh** |`iex (iwr -useb https://aka.ms/kmpkg-init.ps1)`              |` . ~/.kmpkg/kmpkg-init.ps1`          | `rmdir -recurse -force ~/.kmpkg`          |
| **Linux/OSX**       |`. <(curl https://aka.ms/kmpkg-init.sh -L)`                  |` . ~/.kmpkg/kmpkg-init`          | `rm -rf ~/.kmpkg`                  |
| **CMD Shell**       |`curl -LO https://aka.ms/kmpkg-init.cmd && .\kmpkg-init.cmd` |`%USERPROFILE%\.kmpkg\kmpkg-init.cmd` | `rmdir /s /q %USERPROFILE%\.kmpkg` |
...
[main `README.md`](https://github.com/microsoft/kmpkg/blob/master/README.md).
...
Read more about kmpkg telemetry at https://learn.microsoft.com/kmpkg/about/privacy
