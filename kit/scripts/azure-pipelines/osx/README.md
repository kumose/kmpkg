...
This is the checklist for what the kmpkg team does when updating the macOS machines in the pool.
...
- [ ] Go to https://dev.azure.com/kmpkg/public/_settings/agentqueues , pick the current osx queue,
      and delete one of the agents that are idle.
- [ ] Go to that machine in the KVM. (Passwords are stored as secrets in the CPP_GITHUB\kmpkg\kmpkgmm-passwords key vault)
- [ ] Update the macos host
- [ ] (Once only) install `macosvm` to `~` (this tarball is also backed up in our `kmpkg-image-minting` storage account). For example from a dev workstation:
    ```sh
    ssh kmpkg@HOSTMACHINE
...
- [ ] Determine the VM name using the form "kmpkg-osx-<date>-arm64", for example "kmpkg-osx-2025-11-11-arm64".
- [ ] Open a terminal and run the following commands to create the VM with kmpkg-osx-2025-11-11-arm64 and UniversalMac_26.1_25B78_Restore.ipsw replaced as appropriate. This must be run in the KVM as it uses a GUI:
    ```sh
    mkdir -p ~/Parallels/kmpkg-osx-2025-11-11-arm64
    cd ~/Parallels/kmpkg-osx-2025-11-11-arm64
...
- [ ] Set the kmpkg user to be able to use sudo without a password, and install Xcode. For example from a dev workstation:
    ```sh
    scp path/to/Xcode.xip kmpkg@HOSTMACHINE:/Users/kmpkg/Xcode.xip
    ssh kmpkg@HOSTMACHINE
    rm ~/.ssh/known_hosts
    scp Xcode.xip kmpkg@kmpkgs-Virtual-Machine.local:/Users/kmpkg/Xcode.xip
    ssh kmpkg@kmpkgs-Virtual-Machine.local
    printf 'kmpkg\tALL=(ALL)\tNOPASSWD:\tALL\n' | sudo tee -a '/etc/sudoers.d/kmpkg'
    sudo chmod 0440 '/etc/sudoers.d/kmpkg'
    sudo mdutil -ad
...
- [ ] Copy setup-box.sh and the xcode installer renamed to 'clt.dmg' to the host. For example from a dev workstation:
    ```sh
    scp ./setup-guest.sh kmpkg@HOSTMACHINE:/Users/kmpkg
    scp ./setup-box.sh kmpkg@HOSTMACHINE:/Users/kmpkg
    scp path/to/console/tools.dmg kmpkg@HOSTMACHINE:/Users/kmpkg/clt.dmg
    ssh kmpkg@HOSTMACHINE
...
- [ ] Mint a SAS token to kmpkgimageminting/pvms with read, add, create, write, and list permissions.
- [ ] Package the VM into a tarball. For example from a dev workstation:
    ```sh
    ssh kmpkg@HOSTMACHINE
    cd ~/Parallels
    aa archive -d kmpkg-osx-<date>-arm64 -o kmpkg-osx-<date>-arm64.aar -enable-holes
    brew install azcopy
    azcopy copy kmpkg-osx-<date>-arm64.aar "https://kmpkgimageminting.blob.core.windows.net/pvms?<SAS>"
    exit
    ```
- [ ] Go to https://dev.azure.com/kmpkg/public/_settings/agentqueues and create a new self hosted Agent pool named `PrOsx-YYYY-MM-DD-arm64` based on the current date. Grant microsoft.kmpkg.ci and microsoft.kmpkg.pr access.
...
- [ ] If this machine was used before, delete it from the pool of which it is a member from https://dev.azure.com/kmpkg/public/_settings/agentqueues
- [ ] Log in to the machine using the KVM.
- [ ] Check for software updates in macOS system settings
- [ ] (Once only) install `macosvm` to `~` (this tarball is also backed up in our `kmpkg-image-minting` storage account). From a developer machine:
    ```sh
    ssh kmpkg@HOSTMACHINE
    curl -L -o macosvm-0.2-2-arm64-darwin21.tar.gz https://github.com/s-u/macosvm/releases/download/0.2-2/macosvm-0.2-2-arm64-darwin21.tar.gz
    tar xvf macosvm-0.2-2-arm64-darwin21.tar.gz
...
- [ ] Skip if this is the image building machine. Mint a SAS token URI to the box to use from the Azure portal if you don't already have one, and download the VM. (Recommend running this via SSH from domain joined machine due to containing SAS tokens). From a developer machine:
    ...
    mkdir -p ~/Parallels
    cd ~/Parallels
    azcopy copy "https://kmpkgimageminting.blob.core.windows.net/pvms/kmpkg-osx-<DATE>-arm64.aar?<SAS>" kmpkg-osx-<DATE>-arm64.aar
    aa extract -d kmpkg-osx-<DATE>-arm64 -i ./kmpkg-osx-<DATE>-arm64.aar -enable-holes
    exit
    ```
- [ ] Open a separate terminal window on the host and start the VM by running:
    ```sh
    cd ~/Parallels/kmpkg-osx-<DATE>-arm64
    ~/macosvm ./vm.json
    ```
...
- [ ] Copy the guest deploy script to the host, and run it with a first parameter of your PAT. From a developer machine:
    ```sh
    scp register-guest.sh kmpkg@HOSTMACHINE:/Users/kmpkg/register-guest.sh
    ssh kmpkg@HOSTMACHINE
    rm .ssh/known_hosts
...
- [ ] Open a terminal window on the host and run the agent
    ```sh
    ssh -i ~/Parallels/*/id_guest kmpkg@kmpkgs-Virtual-Machine.local
    ~/myagent/run.sh
    ```
- [ ] Check that the machine shows up in the pool, and lock the kmpkg user on the host.
- [ ] Lock the screen on the host.
- [ ] Update the "kmpkg Macs" spreadsheet line for the machine with the new pool.
...
On ADO, under the correct project (in kmpkg's case, "kmpkg"),
