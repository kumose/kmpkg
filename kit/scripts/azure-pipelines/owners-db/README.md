...
- `file_script.ts` — builds databases from a local `info` directory (used in CI runs where kmpkg is installed locally).
...
- `KMPKGDatabase.txt` — a newline-separated list of entries of the form `port:triplet:/path/inside/package`.
- `KMPKGHeadersDatabase.txt` — a newline-separated list of entries of the form `port:triplet:relative/header/path`
...
- `--info-dir <path>` should point at a directory containing kmpkg-generated `.list` files (the same layout created by `kmpkg` under `installed/<triplet>/kmpkg/info/`).
- Each file in that directory is expected to follow the filename convention used by kmpkg info files. The script parses the filename by splitting on underscores and constructs a package identifier using the first and third components:
...
- `--pr-hashes <pr-hashes.json>` is a JSON file produced by the kmpkg tooling that lists changed ports and their ABIs. The script expects a top-level JSON array of objects where each object contains at least the following fields:
...
```sh
npx ts-node ./file_script.ts --info-dir /mnt/kmpkg-ci/installed/kmpkg/info --out-dir ./scripts/list_files
```
...
blob="https://kmpkgbinarycachewus.blob.core.windows.net/cache?${BCACHE_SAS_TOKEN}"
