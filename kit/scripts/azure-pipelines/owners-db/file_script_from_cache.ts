...
function writeOutputLines(outDir: string, dbLines: string[], headerLines: string[]) {
  ...
  fs.writeFileSync(path.join(outDir, "KMPKGDatabase.txt"), dbLines.join("\n") + (dbLines.length ? "\n" : ""));
  fs.writeFileSync(path.join(outDir, "KMPKGHeadersDatabase.txt"), headerLines.join("\n") + (headerLines.length ? "\n" : ""));
}
...
async function main() {
  ...
  // Expect kmpkg-tool produced format: array of objects
  // [ { "name": "zlib", "triplet": "x64-windows", "state": "pass", "abi": "zlib:x64-windows:<sha>" }, ... ]
  if (!Array.isArray(prHashes)) {
    console.error(
      `Invalid pr-hashes.json format: expected a top-level JSON array (kmpkg-tool output).`
    );
    process.exit(2);
  }
  ...
  try {
    ...
    // Find repository root by locating .kmpkg-root in or above cwd
    function findRepoRoot(): string {
      let dir = process.cwd();
      while (true) {
        if (fs.existsSync(path.join(dir, ".kmpkg-root"))) return dir;
        const parent = path.dirname(dir);
        if (parent === dir) break;
        dir = parent;
      }
      throw new Error("Could not find .kmpkg-root in or above current working directory");
    }
  ...
  writeOutputLines(outDir, dbLines, headerLines);
  console.log(`Wrote ${path.join(outDir, "KMPKGDatabase.txt")} and ${path.join(outDir, "KMPKGHeadersDatabase.txt")}`);
}
