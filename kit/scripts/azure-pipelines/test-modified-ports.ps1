...
<#
.SYNOPSIS
Runs the 'Test Modified Ports' part of the kmpkg CI system for all platforms.

.PARAMETER Triplet
The triplet to test.

.PARAMETER WorkingRoot
The location used as scratch space for 'installed', 'packages', and 'buildtrees' kmpkg directories.
...
when using kmpkg to test a prerelease MSVC++ compiler)
...
by the `kmpkg x-check-features` command.
#>
...
function Add-ToolchainToTestCMake {
    # The kmpkg.cmake toolchain file is not part of ABI hashing,
    # but changes must trigger at least some testing.
    Copy-Item "scripts/buildsystems/kmpkg.cmake" -Destination "scripts/test_ports/cmake"
    Copy-Item "scripts/buildsystems/kmpkg.cmake" -Destination "scripts/test_ports/cmake-user"
}
...
if ($testFeatures) {
    & $kmpkgExe x-ci-clean @commonArgs
    $lastLastExitCode = $LASTEXITCODE
    if ($lastLastExitCode -ne 0)
    {
        Write-Error "kmpkg x-ci-clean failed. This is usually an infrastructure problem; trying again may help."
    ...
    & $kmpkgExe x-test-features --for-merge-with origin/master $tripletArg $failureLogsArg $ciBaselineArg $failingAbiLogArg $ciFeatureBaselineArg @commonArgs @cachingArgs
    $lastLastExitCode = $LASTEXITCODE
    ...
    if ($lastLastExitCode -ne 0)
    {
        ...
        Write-Host "##vso[task.logissue type=error]kmpkg feature testing failed; this is usually a bug in one of the features in the port(s) edited in this pull request. See https://github.com/microsoft/kmpkg/discussions/31357 for how to access AZP failure logs."
        exit $lastLastExitCode
    }
...
$toolMetadataFile = "$PSScriptRoot/../kmpkg-tool-metadata.txt"
...
if (($BuildReason -eq 'PullRequest') -and -not $NoParentHashes)
{
    ...
    if (($parentBaseline -eq $headBaseline) -and ($parentTool -eq $headTool))
    {
        ...
        & $kmpkgExe ci $tripletArg --dry-run $ciBaselineArg @commonArgs --no-binarycaching "--output-hashes=$parentHashesFile"
        $lastLastExitCode = $LASTEXITCODE
        if ($lastLastExitCode -ne 0)
        {
            Write-Error "Generating parent hashes failed; this is usually an infrastructure problem with kmpkg"
            exit $lastLastExitCode
        }
    }
    else
...
if ($lastLastExitCode -ne 0)
{
    ...
    {
        Write-Host "##vso[task.logissue type=error]kmpkg ci testing failed; this is usually a bug in a port. See https://github.com/microsoft/kmpkg/discussions/31357 for how to access AZP failure logs."
    }
    else
    {
        Write-Host "##vso[task.logissue type=error]kmpkg ci testing failed, but no build failure logs were created for this error."
    }
}
