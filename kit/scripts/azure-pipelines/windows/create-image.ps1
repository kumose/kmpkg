...
<#
.SYNOPSIS
Creates a Windows virtual machine image, set up for kmpkg's CI.

.DESCRIPTION
create-image.ps1 creates an Azure Windows VM image, set up for kmpkg's CI system.
...
$VirtualNetwork = Get-AzVirtualNetwork -ResourceGroupName 'kmpkg-image-minting' -Name 'kmpkg-image-minting-wus3'
...
$Nic = New-AzNetworkInterface `
  -Name $NicName `
  -ResourceGroupName 'kmpkg-image-minting' `
  -Location $Location `
  -Subnet $Subnet `
...
$VMCreated = Get-AzVM -ResourceGroupName 'kmpkg-image-minting' -Name $ProtoVMName
$VMCreatedOsDisk = $VMCreated.StorageProfile.OsDisk.Name

####################################################################################################
Write-Progress `
  -Activity $ProgressActivity `
  -Status 'Minting token for kmpkg-image-minting storage account' `
  -PercentComplete (100 / $TotalProgress * $CurrentProgress++)

$KmpkgImageMintingAccount = Get-AzStorageAccount -ResourceGroupName 'kmpkg-image-minting' -Name 'kmpkgimageminting'

$AssetStorageContext = New-AzStorageContext -StorageAccountName 'kmpkgimageminting' -UseConnectedAccount
$StartTime = Get-Date
...
Restart-AzVM -ResourceGroupName 'kmpkg-image-minting' -Name $ProtoVMName
...
Wait-Shutdown -ResourceGroupName 'kmpkg-image-minting' -Name $ProtoVMName
...
New-AzGalleryImageVersion `
  -ResourceGroupName 'kmpkg-image-minting' `
  -GalleryName 'kmpkg_gallery_wus3' `
...
Remove-AzDisk -ResourceGroupName 'kmpkg-image-minting' -Name $VMCreatedOsDisk -Force
Remove-AzNetworkInterface -ResourceGroupName 'kmpkg-image-minting' -Name $NicName -Force
