function(kmpkg_from_git)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "kmpkg_from_git was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    if(DEFINED arg_TAG)
        message(WARNING "The TAG argument to kmpkg_from_git has been deprecated and has no effect.")
    endif()
    ...
    kmpkg_list(SET git_fetch_shallow_param --depth 1)
    kmpkg_list(SET extract_working_directory_param)
    kmpkg_list(SET skip_patch_check_param)
    set(git_working_directory "${DOWNLOADS}/git-tmp")
    set(do_download OFF)

    if(KMPKG_USE_HEAD_VERSION AND DEFINED arg_HEAD_REF)
        ...
        if(DEFINED arg_FETCH_REF)
            ...
            kmpkg_list(SET git_fetch_shallow_param)
        else()
...
\"REF a-commit-sha FETCH_REF ${arg_REF}\". (git rev-parse ${expected_rev_parse} failed)")
        ...
        if(KMPKG_USE_HEAD_VERSION)
            set(KMPKG_HEAD_VERSION "${rev_parse_ref}" PARENT_SCOPE)
