...
namespace
{
                               ...
                               PackageSpec spec)
    {
        ...
        return supports_expression.evaluate(var_provider.get_dep_info_vars(spec).value_or_exit(KMPKG_LINE_INFO));
    }

    ActionPlan compute_full_plan(const KmpkgPaths& paths,
                                 ...
                                 const CreateInstallPlanOptions& serialize_options)
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, action_plan.already_installed.empty());
        Checks::check_exit(KMPKG_LINE_INFO, action_plan.remove_actions.empty());
    ...
    std::unordered_set<std::string> parse_parent_hashes(
        const std::map<kmpkg::StringLiteral, std::string, std::less<void>>& settings, const KmpkgPaths& paths)
    {
        ...
        if (it_parent_hashes != settings.end())
        {
            ...
            auto parent_hashes_text = fs.try_read_contents(parent_hashes_path).value_or_exit(KMPKG_LINE_INFO);
            const auto parsed_object =
                Json::parse(parent_hashes_text.content, parent_hashes_text.origin).value_or_exit(KMPKG_LINE_INFO);
            const auto& parent_hashes_array = parsed_object.value.array(KMPKG_LINE_INFO);
            for (const Json::Value& array_value : parent_hashes_array)
            {
                auto abi = array_value.object(KMPKG_LINE_INFO).get(JsonIdAbi);
                Checks::check_exit(KMPKG_LINE_INFO, abi);
                parent_hashes.insert(abi->string(KMPKG_LINE_INFO).to_string());
            }
        }
...
namespace kmpkg
{
                             ...
                             Triplet host_triplet)
    {
        ...
        if (baseline_iter == settings.end())
        {
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgCiBaselineAllowUnexpectedPassingRequiresBaseline);
            }
        }
        else
        {
            ...
            const auto ci_baseline_file_contents = fs.read_contents(*ci_baseline_file_name, KMPKG_LINE_INFO);
        ...
        if (it_known_failures != settings.end())
        {
            ...
            auto lines = paths.get_filesystem().read_lines(raw_path).value_or_exit(KMPKG_LINE_INFO);
        ...
        Optional<CiBuildLogsRecorder> build_logs_recorder_storage;
        {
            ...
            if (it_failure_logs != settings.end())
            {
                ...
                build_logs_recorder = &(build_logs_recorder_storage.emplace(
                    fs.almost_canonical(raw_path, KMPKG_LINE_INFO), fs.file_time_now()));
            }
        }
        ...
        if (it_output_hashes != settings.end())
        {
            ...
            fs.write_contents(output_hash_json, Json::stringify(pre_build_status.abis), KMPKG_LINE_INFO);
        }
        ...
        else
        {
            ...
            for (auto&& result : summary.results)
            {
                if (const auto* ipa = result.get_maybe_install_plan_action())
                {
                    ...
                    auto ci_result = CiResult{result.build_result.value_or_exit(KMPKG_LINE_INFO).code,
        ...
        if (it_xunit != settings.end())
        {
            ...
            fs.write_contents(it_xunit->second, xunitTestResults.build_xml(target_triplet), KMPKG_LINE_INFO);
        }
