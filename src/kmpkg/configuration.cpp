...
namespace
{
    ...
    struct CeMetadataDeserializer final : Json::IDeserializer<Json::Object>
    {
        virtual LocalizedString type_name() const override
        {
            return msg::format(msgAnObjectContainingKmpkgArtifactsMetadata);
        }
        ...
        for (const auto& el : obj)
        {
            ...
            if (el.first == JsonIdDemands)
            {
                ...
                for (const auto& demand : *maybe_demands_object)
                {
                    ...
                    find_unknown_fields_impl(demand.second.object(KMPKG_LINE_INFO),
                                             out,
...
namespace kmpkg
{
    static ExpectedL<Optional<std::string>> get_baseline_from_git_repo(const KmpkgPaths& paths,
                                                                       StringView url,
    ...
    ExpectedL<Optional<std::string>> RegistryConfig::get_latest_baseline(const KmpkgPaths& paths) const
    {
        if (kind == JsonIdGit)
        {
            return get_baseline_from_git_repo(paths, repo.value_or_exit(KMPKG_LINE_INFO), reference.value_or("HEAD"));
        }
        else if (kind == JsonIdBuiltin)
        {
            ...
            {
                // use the kmpkg git repository sha from the user's machine
    ...
    StringView RegistryConfig::pretty_location() const
    {
        ...
        {
            return path.value_or_exit(KMPKG_LINE_INFO);
        }
        if (kind == JsonIdGit)
        {
            return repo.value_or_exit(KMPKG_LINE_INFO);
        }
        if (kind == JsonIdArtifact)
        {
            return location.value_or_exit(KMPKG_LINE_INFO);
        }
    ...
    StringLiteral configuration_source_file_name(ConfigurationSource source)
    {
        switch (source)
        {
            ...
            case ConfigurationSource::ManifestFileConfiguration: return FileKmpkgDotJson;
            ...
            case ConfigurationSource::KmpkgConfigurationFile: return FileKmpkgConfigurationDotJson;
    ...
    StringLiteral configuration_source_field(ConfigurationSource source)
    {
        switch (source)
        {
            ...
            case ConfigurationSource::KmpkgConfigurationFile: return "";
            case ConfigurationSource::ManifestFileKmpkgConfiguration: return JsonIdKmpkgConfiguration;
    ...
    Optional<Configuration> parse_configuration(StringView contents, StringView origin, MessageSink& messageSink)
    {
        ...
        auto conf_value = std::move(conf).value(KMPKG_LINE_INFO).value;
    ...
    static std::unique_ptr<RegistryImplementation> instantiate_rconfig(const KmpkgPaths& paths,
                                                                       ...
                                                                       const Path& config_dir)
    {
        if (auto k = config.kind.get())
        {
            if (*k == JsonIdBuiltin)
            {
                return make_builtin_registry(paths, config.baseline.value_or_exit(KMPKG_LINE_INFO));
            }
            else if (*k == JsonIdGit)
            {
                return make_git_registry(paths,
                                         config.repo.value_or_exit(KMPKG_LINE_INFO),
                                         config.reference.value_or("HEAD"),
                                         config.baseline.value_or_exit(KMPKG_LINE_INFO));
            }
            else if (*k == JsonIdFilesystem)
            {
                return make_filesystem_registry(paths.get_filesystem(),
                                                config_dir / config.path.value_or_exit(KMPKG_LINE_INFO),
    ...
    std::unique_ptr<RegistrySet> Configuration::instantiate_registry_set(const KmpkgPaths& paths,
