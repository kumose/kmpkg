#include <kmpkg/base/api-stable-format.h>
...
#include <kmpkg/versiondeserializers.h>
...
namespace kmpkg
{
    ...
    struct DependencyOverrideDeserializer final : Json::IDeserializer<DependencyOverride>
    {
        ...
        virtual View<StringLiteral> valid_fields() const noexcept override
        {
            static constexpr StringLiteral fields[] = {KMPKG_SCHEMED_DESERIALIZER_FIELDS, JsonIdName};
            return fields;
        }
...
#include "spdx-exceptions.inc"
    ...
    //   EBNF located at: https://learn.microsoft.com/kmpkg/reference/kmpkg-json#license
    ...
    {
        virtual LocalizedString type_name() const override { return msg::format(msgAKmpkgRepositoryCommit); }

        virtual Optional<std::string> visit_string(Json::Reader&, StringView s) const override
        {
            // We allow non-sha strings here to allow the core kmpkg code to provide better error
    ...
    struct ManifestDeserializer : Json::IDeserializer<std::unique_ptr<SourceControlFile>>
    {
        ...
        virtual View<StringLiteral> valid_fields() const noexcept override
        {
            static constexpr StringLiteral fields[] = {
                KMPKG_SCHEMED_DESERIALIZER_FIELDS,
                ...
                JsonIdSummary,
                JsonIdSupports,
                JsonIdKmpkgConfiguration,
            };

            return fields;
        }

        kmpkg::Optional<std::unique_ptr<kmpkg::SourceControlFile>> visit_object_common(
            const kmpkg::Json::Object& obj,
            kmpkg::SourceParagraph& spgh,
            kmpkg::Json::Reader& r,
            std::unique_ptr<kmpkg::SourceControlFile>& control_file) const
        {
            for (const auto& el : obj)
            {
                if (el.first.starts_with("$"))
            ...
            auto kmpkg_configuration = obj.get(JsonIdKmpkgConfiguration);
            if (kmpkg_configuration)
            {
                if (auto configuration_object = kmpkg_configuration->maybe_object())
                {
                    spgh.configuration.emplace(*configuration_object);
                    spgh.configuration_source = ConfigurationSource::ManifestFileKmpkgConfiguration;
                }
                else
                {
                    r.add_generic_error(type_name(),
                                        msg::format(msgJsonFieldNotObject, msg::json_field = JsonIdKmpkgConfiguration));
                }
            }
            ...
            if (auto error = maybe_error.get())
            {
                Checks::msg_exit_with_message(KMPKG_LINE_INFO, *error);
            }
    ...
    struct ManifestConfigurationDeserializer final : Json::IDeserializer<ManifestConfiguration>
    {
        ...
        virtual Optional<ManifestConfiguration> visit_object(Json::Reader& r, const Json::Object& obj) const override
        {
            ...
            if (r.optional_object_field_emplace(obj, JsonIdKmpkgConfiguration, ret.config, configuration_deserializer))
            {
                ret.config_source = ConfigurationSource::ManifestFileKmpkgConfiguration;
                                                                  ...
                                                                  MessageSink& warningsSink)
    {
        ...
        return std::move(res).value_or_exit(KMPKG_LINE_INFO);
    }
    ...
    std::vector<FullPackageSpec> filter_dependencies(const std::vector<kmpkg::Dependency>& deps,
                                                     Triplet target,
                                                     Triplet host,
    ...
    ParsedSpdxLicenseDeclaration::ParsedSpdxLicenseDeclaration(NullTag)
        ...
        , m_license_text(SpdxLicenseRefKmpkgNull.data(), SpdxLicenseRefKmpkgNull.size())
    ...
    Json::Object serialize_manifest(const SourceControlFile& scf)
    {
        ...
        if (auto configuration = scf.core_paragraph->configuration.get())
        {
            ...
            obj.insert(source_field, maybe_configuration.value_or_exit(KMPKG_LINE_INFO).serialize());
        }
        ...
        if (scf.core_paragraph->builtin_baseline.has_value())
        {
            obj.insert(JsonIdBuiltinBaseline,
                       Json::Value::string(scf.core_paragraph->builtin_baseline.value_or_exit(KMPKG_LINE_INFO)));
        }
