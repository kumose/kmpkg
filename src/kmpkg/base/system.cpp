...
namespace
{
    ...
    struct HKey
    {
        ...
        ExpectedL<RegistryValue> query_value(StringView valuename) const
        {
            if (hkey == INVALID_HKEY_VALUE)
            {
                Checks::unreachable(KMPKG_LINE_INFO, "Tried to query invalid key");
            }
    ...
    // keep this in sync with kmpkg-tools.schema.json
...
namespace kmpkg
{
    ...
    StringLiteral to_string_literal(CPUArchitecture arch) noexcept
    {
        ...
        Checks::unreachable(KMPKG_LINE_INFO, "unexpected kmpkg::CPUArchitecture");
    }
...
#error "Unknown host architecture"
    ...
    Optional<std::string> get_environment_variable(ZStringView varname) noexcept
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, MAXDWORD >= ret.size());
        const auto sz2 = GetEnvironmentVariableW(w_varname.c_str(), ret.data(), static_cast<DWORD>(ret.size()));
        Checks::check_exit(KMPKG_LINE_INFO, sz2 + 1 == sz);
        ret.pop_back();
    ...
    void set_environment_variable(ZStringView varname, Optional<ZStringView> value) noexcept
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, exit_code != 0);
#else
        if (auto v = value.get())
        {
            Checks::check_exit(KMPKG_LINE_INFO, setenv(varname.c_str(), v->c_str(), 1) == 0);
        }
        else
        {
            Checks::check_exit(KMPKG_LINE_INFO, unsetenv(varname.c_str()) == 0);
        }
#endif
    }
    ...
    std::vector<std::string> get_environment_variables()
    {
        ...
        {
            LPWCH strings;
            EnvironmentStringsW() : strings(GetEnvironmentStringsW()) { Checks::check_exit(KMPKG_LINE_INFO, strings); }
...
#undef HOMEVAR
    ...
    {
        // This needs to be lowercase or msys-ish tools break. See https://github.com/kumose/kmpkg-tool/pull/418/
    ...
    const ExpectedL<Path>& get_platform_cache_kmpkg() noexcept
    {
        static ExpectedL<Path> s_kmpkg = get_platform_cache_root().map([](const Path& p) { return p / "kmpkg"; });
        return s_kmpkg;
    }
    ...
    const ExpectedL<Path>& get_user_configuration_home() noexcept
    {
        ...
        static const ExpectedL<Path> result =
            get_appdata_local().map([](const Path& appdata_local) { return appdata_local / "kmpkg"; });
#else
        static const ExpectedL<Path> result = Path(get_environment_variable("HOME").value_or("/var")) / ".kmpkg";
#endif
        return result;
    }
    ...
    void reset_processor_architecture_environment_variable()
    {
        ...
        switch (proc)
        {
            ...
            default:
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgUnexpectedWindowsArchitecture, msg::actual = proc);
                break;
        }

        Checks::check_exit(KMPKG_LINE_INFO, SetEnvironmentVariableW(L"PROCESSOR_ARCHITECTURE", value) != 0);
    }
#endif
    ...
    unsigned int get_concurrency()
    {
        static unsigned int concurrency = [] {
            auto user_defined_concurrency = get_environment_variable(EnvironmentVariableKmpkgMaxConcurrency);
            if (user_defined_concurrency)
            {
                int res = -1;
                try
                {
                    res = std::stoi(user_defined_concurrency.value_or_exit(KMPKG_LINE_INFO));
                }
                catch (std::exception&)
                {
                    Checks::msg_exit_with_message(
                        KMPKG_LINE_INFO, msgOptionMustBeInteger, msg::option = EnvironmentVariableKmpkgMaxConcurrency);
                }
                ...
                if (!(res > 0))
                {
                    Checks::msg_exit_with_message(KMPKG_LINE_INFO,
                                                  ...
                                                  msg::env_var = EnvironmentVariableKmpkgMaxConcurrency,
                                                  msg::value = res);
                }
    ...
    Optional<CPUArchitecture> guess_visual_studio_prompt_target_architecture()
    {
        ...
        {
            return to_cpu_architecture(vscmd_arg_tgt_arch_env.value_or_exit(KMPKG_LINE_INFO));
        }
        ...
        if (get_environment_variable(EnvironmentVariableVCInstallDir))
        {
            ...
            if (Platform)
            {
                return to_cpu_architecture(Platform.value_or_exit(KMPKG_LINE_INFO));
            }
            else
            {
                return CPUArchitecture::X86;
