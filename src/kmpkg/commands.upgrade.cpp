...
#include <kmpkg/commands.upgrade.h>
...
namespace kmpkg
{
    constexpr CommandMetadata CommandUpgradeMetadata = {
        ...
        {"kmpkg upgrade --no-dry-run"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/upgrade",
                                  ...
                                  Triplet host_triplet)
    {
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgUpgradeInManifest);
        }
        ...
        else
        {
            ...
            for (auto&& spec : specs)
            {
                ...
                const auto& control_file = maybe_control_file.value_or_exit(KMPKG_LINE_INFO);
        ...
        Checks::check_exit(KMPKG_LINE_INFO, !action_plan.empty());
