...
#include <kmpkg/commands.remove.h>
#include <kmpkg/commands.set-installed.h>
...
namespace kmpkg
{
                                          ...
                                          const SymlinkHydrate hydrate)
    {
        ...
        fs.create_directories(destination_triplet, KMPKG_LINE_INFO);
        const auto listfile_parent = listfile.parent_path();
        fs.create_directories(listfile_parent, KMPKG_LINE_INFO);
        ...
        for (auto&& proximate_file : proximate_files)
        {
            ...
            switch (status)
            {
                ...
                {
                    if (filename == FileControl || filename == FileKmpkgDotJson || filename == FileBuildInfo)
                    {
        ...
        fs.write_lines(listfile, listfile_lines, KMPKG_LINE_INFO);
    }
    ...
    static InstallResult install_package(const KmpkgPaths& paths,
    ...
    static ExtendedBuildResult perform_install_plan_action(const KmpkgCmdArguments& args,
        ...
        if (plan_type == InstallPlanType::BUILD_AND_INSTALL)
        {
            ...
            if (binary_cache.is_restored(action))
            {
                auto maybe_bcf = Paragraphs::try_load_cached_package(
                    fs, action.package_dir.value_or_exit(KMPKG_LINE_INFO), action.spec);
                bcf = std::make_unique<BinaryControlFile>(std::move(maybe_bcf).value_or_exit(KMPKG_LINE_INFO));
            ...
            Checks::check_exit(KMPKG_LINE_INFO, bcf != nullptr);
            BuildResult code;
            if (all_dependencies_satisfied)
            {
                const auto install_result =
                    install_package(paths, action.package_dir.value_or_exit(KMPKG_LINE_INFO), *bcf, status_db);
            ...
            {
                Checks::check_exit(KMPKG_LINE_INFO, build_options.only_downloads == OnlyDownloads::Yes);
    ...
    LocalizedString InstallSummary::format_results() const
    {
        ...
        {
            summary_counts[r.get_spec().triplet()].increment(r.build_result.value_or_exit(KMPKG_LINE_INFO).code);
    ...
    void InstallSummary::print_failed() const
    {
        ...
        {
            if (result.build_result.value_or_exit(KMPKG_LINE_INFO).code != BuildResult::Succeeded)
            {
    ...
    void install_preclear_plan_packages(const KmpkgPaths& paths, const ActionPlan& action_plan)
    {
        purge_packages_dirs(paths, action_plan.remove_actions);
        install_clear_installed_packages(paths, action_plan.install_actions);
    }

    void install_clear_installed_packages(const KmpkgPaths& paths, View<InstallPlanAction> install_actions)
    {
        auto& fs = paths.get_filesystem();
        for (auto&& action : install_actions)
        {
            fs.remove_all(action.package_dir.value_or_exit(KMPKG_LINE_INFO), KMPKG_LINE_INFO);
        }
    }

    InstallSummary install_execute_plan(const KmpkgCmdArguments& args,
                                        const KmpkgPaths& paths,
                                        Triplet host_triplet,
                                        const BuildPackageOptions& build_options,
            ...
            if (result.code == BuildResult::Succeeded)
            {
                if (auto scfl = action.source_control_file_and_location.get())
                {
                    ...
                    for (const auto& feature_name : action.feature_list)
                    {
                        ...
                        const auto& feature = scf.find_feature(feature_name).value_or_exit(KMPKG_LINE_INFO);
            ...
            else if (build_options.keep_going == KeepGoing::No)
            {
                ...
                print_user_troubleshooting_message(
                    ...
                    result.stdoutlog.then([&](auto&) -> Optional<Path> {
                        auto issue_body_path = paths.installed().root() / FileKmpkg / FileIssueBodyMD;
    ...
    static std::vector<std::string> get_all_known_reachable_port_names_no_network(const KmpkgPaths& paths)
    {
    ...
    constexpr CommandMetadata CommandInstallMetadata{
        ...
        {msgCmdInstallExample1,
         "kmpkg install zlib zlib:x64-windows curl boost",
         "kmpkg install --triplet x64-windows"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/install",
    ...
    constexpr CommandMetadata CommandInstallMetadataClassic{
        "install",
        msgHelpInstallCommand,
        {msgCmdInstallExample1, "kmpkg install zlib zlib:x64-windows curl boost"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/install",
        AutocompletePriority::Public,
        0,
        SIZE_MAX,
        ...
        if (auto files = maybe_files.get())
        {
            ...
            for (auto&& triplet_and_suffix : *files)
            {
                ...
                else if (suffix.starts_with("share/") && suffix.ends_with(DOT_CMAKE))
                {
                    ...
                    if (suffix_without_ending.ends_with("/kmpkg-port-config")) continue;
                    if (suffix_without_ending.ends_with("/kmpkg-cmake-wrapper")) continue;
            ...
            if (!pkgconfig_files.empty())
            {
                ...
                {
                    const auto lines = fs.read_lines(path).value_or_exit(KMPKG_LINE_INFO);
    ...
    static bool cmake_args_sets_variable(const KmpkgCmdArguments& args)
    {
        return Util::any_of(args.cmake_args, [](StringView s) { return s.starts_with("-D"); });
    }

    void command_install_and_exit(const KmpkgCmdArguments& args,
                                  const KmpkgPaths& paths,
                                  ...
                                  Triplet host_triplet)
    {
        ...
        if (manifest)
        {
            ...
            auto manifest_scf = std::move(maybe_manifest_scf).value(KMPKG_LINE_INFO);
        ...
        if (auto common_triplet = maybe_common_triplet.get())
        {
            ...
            if (auto common_arch = maybe_common_arch.get())
            {
                ...
                if (auto vs_prompt = maybe_vs_prompt.get())
                {
                    ...
                    {
                        msg::println_warning(
                            msgKmpkgInVsPrompt, msg::value = *vs_prompt, msg::triplet = *common_triplet);
                    }
                }
            }
        }
        ...
        if (it_pkgsconfig != options.settings.end())
        {
            ...
            fs.write_contents(pkgsconfig_path, pkgsconfig_contents, KMPKG_LINE_INFO);
        ...
        if (it_xunit != options.settings.end())
        {
            ...
            for (auto&& result : summary.results)
            {
                if (const auto* ipa = result.get_maybe_install_plan_action())
                {
                    xwriter.add_test_results(result.get_spec(),
                                             CiResult{result.build_result.value_or_exit(KMPKG_LINE_INFO).code,
                                                      CiBuiltResult{ipa->package_abi_or_exit(KMPKG_LINE_INFO),
                                                                    ipa->feature_list,
                                                                    result.start_time,
                                                                    result.timing}});
                }
            ...
            fs.write_contents(it_xunit->second, xwriter.build_xml(default_triplet), KMPKG_LINE_INFO);
        }
        ...
        Checks::exit_with_code(KMPKG_LINE_INFO, summary.failed);
    }
    ...
    void SpecSummary::to_string(std::string& out_str) const
    {
        ...
        out_str.append(": ");
        out_str.append(kmpkg::to_string(build_result.value_or_exit(KMPKG_LINE_INFO).code).data());
        out_str.append(": ");
    ...
    void track_install_plan(const ActionPlan& plan)
    {
        ...
        for (auto&& install_action : plan.install_actions)
        {
            auto&& version_as_string =
                install_action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).to_version().to_string();
            if (!specs_string.empty()) specs_string.push_back(',');
