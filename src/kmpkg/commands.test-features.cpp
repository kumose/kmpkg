...
#include <kmpkg/ci-feature-baseline.h>
...
#include <kmpkg/commands.test-features.h>
...
namespace
{
    ...
    std::vector<std::string> get_for_merge_with_test_port_names(const KmpkgPaths& paths, StringView for_merge_with)
    {
        ...
        auto ports_dir_prefix =
            git_prefix(console_diagnostic_context, git_exe, builtin_ports).value_or_quiet_exit(KMPKG_LINE_INFO);
        const auto locator = GitRepoLocator{GitRepoLocatorKind::CurrentDirectory, builtin_ports};
        auto index_file =
            git_index_file(console_diagnostic_context, fs, git_exe, locator).value_or_quiet_exit(KMPKG_LINE_INFO);
        TempFileDeleter temp_index_file{fs, fmt::format("{}_kmpkg_{}.tmp", index_file.native(), get_process_id())};
        if (!fs.copy_file(
        ...
        auto head_tree = git_write_index_tree(console_diagnostic_context, git_exe, locator, temp_index_file.path)
                             .value_or_quiet_exit(KMPKG_LINE_INFO);
                                                         ...
                                                         PathsPortFileProvider& provider)
    {
        return Util::fmap(test_port_names, [&](const std::string& arg) {
            return provider.get_control_file(arg).value_or_exit(KMPKG_LINE_INFO).source_control_file.get();
        });
    }
...
KMPKG_FORMAT_WITH_TO_STRING(SpecToTest);
...
namespace kmpkg
{
    ...
    constexpr CommandMetadata CommandTestFeaturesMetadata = {
        ...
        {"kmpkg x-test-features gdal"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/test-features",
                                        ...
                                        Triplet host_triplet)
    {
        ...
        Optional<Path> maybe_build_logs_base_path;
        {
            ...
            if (it_failure_logs != settings.end())
            {
                ...
                fs.create_directories(raw_path, KMPKG_LINE_INFO);
                maybe_build_logs_base_path.emplace(fs.almost_canonical(raw_path, KMPKG_LINE_INFO));
            }
        }
        ...
        if (all_ports)
        {
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgMutuallyExclusivePorts, msg::option = SwitchAll);
            }
        ...
        {
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgMutuallyExclusivePorts, msg::option = SwitchForMergeWith);
        }
        ...
        if (feature_baseline_iter != settings.end())
        {
            ...
            ci_feature_baseline_file_contents = fs.read_contents(*ci_feature_baseline_file_name, KMPKG_LINE_INFO);
            ...
            if (test_spec.plan.unsupported_features.empty())
            {
                for (auto& actions : test_spec.plan.install_actions)
                {
                    ...
                    port_locations.emplace_back(
                        actions.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).port_directory());
                }
        ...
        for (std::size_t i = 0; i < specs_to_test.size(); ++i)
        {
            ...
            if (auto iter = Util::find_if(install_plan.install_actions,
                                          [&known_failures](const auto& install_action) {
                                              return Util::Sets::contains(
                                                  known_failures, install_action.package_abi_or_exit(KMPKG_LINE_INFO));
                                          });
...

            {
                    ...
                    CacheAvailability::available)
                {
                    msg::println(msgSkipTestingOfPortAlreadyInBinaryCache,
                                 msg::sha = action->package_abi_or_exit(KMPKG_LINE_INFO));
                    handle_pass_feature_test_result(diagnostics, spec, ci_feature_baseline_file_name, baseline);
                    continue;
                }
            }
            ...
            {
                auto& build_result = result.build_result.value_or_exit(KMPKG_LINE_INFO);
                switch (build_result.code)
                {
                    case BuildResult::BuildFailed:
                        if (Path* logs_dir = maybe_logs_dir.get())
                        {
                            ...
                            Checks::check_exit(KMPKG_LINE_INFO, ipa);
                    ...
                    case BuildResult::PostBuildChecksFailed:
                        known_failures.insert(result.package_abi_or_exit(KMPKG_LINE_INFO));
                        break;
                    default: break;
                }
            }

            auto& last_build_result = summary.results.back().build_result.value_or_exit(KMPKG_LINE_INFO);
            switch (last_build_result.code)
            {
                ...
                case BuildResult::CacheMissing:
                    ...
                    if (maybe_logs_dir)
                    {
                        fs.create_directories(*maybe_logs_dir.get(), KMPKG_LINE_INFO);
                        fs.write_contents(
                            *maybe_logs_dir.get() / FileTestedSpecDotTxt, spec.to_string(), KMPKG_LINE_INFO);
                    }
        ...
        if (it_output_file != settings.end())
        {
            ...
            content += '\n';
            fs.write_contents_and_dirs(raw_path, content, KMPKG_LINE_INFO);
        }

        binary_cache.wait_for_async_complete_and_join();

        Checks::exit_with_code(KMPKG_LINE_INFO, exit_code);
    }
}
