...
namespace kmpkg
{
    constexpr CommandMetadata CommandNewMetadata{
        "new",
        msgCmdNewSynposis,
        {msgCmdNewExample1, "kmpkg new --application"},
        Undocumented,
        ...
        if (name)
        {
            ...
            if (!Json::IdentifierDeserializer::is_ident(*name))
            {
                return msg::format_error(msgParseIdentifierError,
                                         msg::value = *name,
                                         msg::url = "https://pub.kumose.cc/kmdo/commands/kmpkg/new");
            }
    ...
    void command_new_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        auto& manifest = maybe_manifest.value_or_exit(KMPKG_LINE_INFO);

        const auto almost_original_cwd = fs.almost_canonical(paths.original_cwd, KMPKG_LINE_INFO);
        const auto candidate_manifest_path = almost_original_cwd / FileKmpkgDotJson;
        const auto candidate_configuration_path = almost_original_cwd / FileKmpkgConfigurationDotJson;
        if (fs.exists(candidate_manifest_path, KMPKG_LINE_INFO))
        {
            Checks::msg_exit_with_error(
                KMPKG_LINE_INFO, msgNewManifestAlreadyExists, msg::path = candidate_manifest_path);
        }

        if (fs.exists(candidate_configuration_path, KMPKG_LINE_INFO))
        {
            Checks::msg_exit_with_error(
                KMPKG_LINE_INFO, msgNewConfigurationAlreadyExists, msg::path = candidate_configuration_path);
        }
        ...
        if (configuration.registries.empty())
        {
            ...
            default_ms_registry.location.emplace(
                "https://github.com/kumose/kmpkg-ce-catalog/archive/refs/heads/main.zip");
            configuration.registries.emplace_back(std::move(default_ms_registry));
        }

        if (option_single_file)
        {
            manifest.insert(JsonIdKmpkgConfiguration, configuration.serialize());
        }
        else
        {
            fs.write_contents(
                candidate_configuration_path, Json::stringify(configuration.serialize()), KMPKG_LINE_INFO);
        }

        fs.write_contents(candidate_manifest_path, Json::stringify(manifest), KMPKG_LINE_INFO);
