...
#include <kmpkg/commands.depend-info.h>
...
namespace
{
                        ...
                        std::set<std::string>& printed)
    {
        ...
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgExceededRecursionDepth);
        }
        auto currPos = std::find_if(
            allDepends.begin(), allDepends.end(), [&currDepend](const auto& p) { return p.package == currDepend; });
        Checks::check_exit(KMPKG_LINE_INFO, currPos != allDepends.end(), "internal kmpkg error");
                                      ...
                                      const std::map<std::string, PackageDependInfo&>& dependencies_map)
    {
        for (auto it = packages.rbegin(), last = packages.rend(); it != last; ++it)
        {
            ...
            for (const auto& dependency : it->dependencies)
            {
                ...
                {
                    Checks::unreachable(KMPKG_LINE_INFO, fmt::format("Not found in dependency graph: {}", dependency));
                }
...
namespace kmpkg
{
    ...
    constexpr CommandMetadata CommandDependInfoMetadata{
        ...
        {msgCmdDependInfoExample1, "kmpkg depend-info zlib"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/depend-info",
    ...
    void command_depend_info_and_exit(const KmpkgCmdArguments& args,
                                      ...
                                      Triplet host_triplet)
    {
        ...
        const auto strategy = determine_depend_info_mode(options).value_or_exit(KMPKG_LINE_INFO);
        ...
        {
            Checks::unreachable(KMPKG_LINE_INFO, "Only install actions should exist in the plan");
        }
