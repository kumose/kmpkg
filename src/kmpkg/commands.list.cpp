...
namespace
{
    void do_print_json(std::vector<const kmpkg::StatusParagraph*> installed_packages)
    {
        Json::Object obj;
        for (const StatusParagraph* status_paragraph : installed_packages)
        {
            ...
            if (obj.contains(current_spec_string))
            {
                if (status_paragraph->package.is_feature())
                {
                    ...
                    auto& feature_list = value_obj->object(KMPKG_LINE_INFO)[JsonIdFeatures].array(KMPKG_LINE_INFO);
    ...
    void do_print(const StatusParagraph& pgh, const bool full_desc)
    {
        ...
        else
        {
            ...
            msg::write_unlocalized_text_to_stdout(Color::none,
                                                  fmt::format("{:<50}{:<20}{:<}\n",
                                                              kmpkg::shorten_text(pgh.package.display_name(), 50),
                                                              kmpkg::shorten_text(full_version, 16),
                                                              kmpkg::shorten_text(description, 51)));
        }
    }
...
namespace kmpkg
{
    constexpr CommandMetadata CommandListMetadata{
        "list",
        msgListHelp,
        {"kmpkg list", msgCmdListExample2, "kmpkg list png"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/list",
