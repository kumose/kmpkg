...
#include <kmpkg/commands.integrate.h>
...
namespace kmpkg
{
    ...
    std::vector<std::string> get_bash_source_completion_lines(StringView contents)
    {
        ...
        while (first != last)
        {
            ...
            if (line.starts_with("source") && line.ends_with("scripts/kmpkg_completion.bash"))
            {
    ...
    ZshAutocomplete get_zsh_autocomplete_data(StringView contents)
    {
        ...
        while (first != last)
        {
            ...
            if (line.starts_with("source") && line.ends_with("scripts/kmpkg_completion.zsh"))
            {
...
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <Import Condition="Exists('{}') and '$(KMPkgLocalAppDataDisabled)' == ''" Project="{}" />
</Project>
)###",
                           target_path,
                           target_path);
...
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <!-- version 1 -->
  <PropertyGroup>
    <VCLibPackagePath Condition="'$(VCLibPackagePath)' == ''">$(LOCALAPPDATA)\kmpkg\kmpkg.user</VCLibPackagePath>
  </PropertyGroup>
  ...
  <Import Condition="'$(VCLibPackagePath)' != '' and Exists('$(VCLibPackagePath).targets')" Project="$(VCLibPackagePath).targets" />
    ...
    static std::string create_nuget_targets_file_contents(const Path& msbuild_kmpkg_targets_file)
    {
        return fmt::format(R"###(
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <Import Project="{}" Condition="Exists('{}')" />
  <Target Name="CheckValidPlatform" BeforeTargets="Build">
    <Error Text="Unsupported architecture combination. Remove the 'kmpkg' nuget package." Condition="'$(KMPkgEnabled)' != 'true' and '$(KMPkgDisableError)' == ''"/>
  </Target>
</Project>
)###",
                           msbuild_kmpkg_targets_file,
                           msbuild_kmpkg_targets_file);
    }
#endif // ^^^ _WIN32
...
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <PropertyGroup>
    <KMPkgLocalAppDataDisabled>true</KMPkgLocalAppDataDisabled>
  </PropertyGroup>
</Project>
)###";
#endif // ^^^ _WIN32
...
#if defined(_WIN32)
    static std::string get_nuget_id(const Path& kmpkg_root_dir)
    {
        std::string dir_id = kmpkg_root_dir.generic_u8string();
...
#if defined(_WIN32)
    static std::string create_nuspec_file_contents(const Path& kmpkg_root_dir,
...
<package>
    <metadata>
        ...
        <description>
            This package imports all libraries currently installed in @KMPKG_DIR@. This package does not contain any libraries and instead refers to the folder directly (like a symlink).
        </description>
    </metadata>
    <files>
        <file src="kmpkg.nuget.props" target="build\native\@NUGET_ID@.props" />
        <file src="kmpkg.nuget.targets" target="build\native\@NUGET_ID@.targets" />
    </files>
</package>
)";

        std::string content = Strings::replace_all(CONTENT_TEMPLATE, "@NUGET_ID@", nuget_id);
        Strings::inplace_replace_all(content, "@KMPKG_DIR@", kmpkg_root_dir);
    ...
    static bool integrate_install_msbuild14(const Filesystem& fs)
    {
        Path OLD_SYSTEM_TARGET_FILES[] = {
            get_program_files_32_bit().value_or_exit(KMPKG_LINE_INFO) /
                "MSBuild/14.0/Microsoft.Common.Targets/ImportBefore/kmpkg.nuget.targets",
            get_program_files_32_bit().value_or_exit(KMPKG_LINE_INFO) /
                "MSBuild/14.0/Microsoft.Common.Targets/ImportBefore/kmpkg.system.targets"};

        Path SYSTEM_WIDE_TARGETS_FILE = get_program_files_32_bit().value_or_exit(KMPKG_LINE_INFO) /
                                        "MSBuild/Microsoft.Cpp/v4.0/V140/ImportBefore/Default/kmpkg.system.props";
        ...
        const auto tmp_dir = fs.create_or_get_temp_directory(KMPKG_LINE_INFO);
        const auto sys_src_path = tmp_dir / "kmpkg.system.targets";
        fs.write_contents(sys_src_path, SystemTargetsShortcut, KMPKG_LINE_INFO);
        ...
        fs.remove_all(tmp_dir, KMPKG_LINE_INFO);
    ...
    static void integrate_install(const KmpkgPaths& paths)
    {
        auto& fs = paths.get_filesystem();

        const auto cmake_toolchain = paths.buildsystems / "kmpkg.cmake";
        auto message = msg::format(msgCMakeToolChainFile, msg::path = cmake_toolchain.generic_u8string());

        auto& user_configuration_home = get_user_configuration_home().value_or_exit(KMPKG_LINE_INFO);
        fs.create_directories(user_configuration_home, KMPKG_LINE_INFO);
        fs.write_contents(user_configuration_home / FileKmpkgPathTxt, paths.root.generic_u8string(), KMPKG_LINE_INFO);

#if defined(_WIN32)
        fs.write_contents(user_configuration_home / FileKmpkgUserProps,
                          create_appdata_shortcut(paths.buildsystems_msbuild_props),
                          KMPKG_LINE_INFO);
        fs.write_contents(user_configuration_home / FileKmpkgUserTargets,
        ...
        if (!integrate_install_msbuild14(fs))
        {
            ...
            msg::println(message);
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgIntegrationFailedVS2015);
        }
    ...
    static void integrate_remove(const Filesystem& fs)
    {
        ...
        auto& user_configuration_home = get_user_configuration_home().value_or_exit(KMPKG_LINE_INFO);
#if defined(_WIN32)
        was_deleted |= fs.remove(user_configuration_home / FileKmpkgUserProps, KMPKG_LINE_INFO);
        was_deleted |= fs.remove(user_configuration_home / FileKmpkgUserTargets, KMPKG_LINE_INFO);
#endif // ^^^ _WIN32
        was_deleted |= fs.remove(user_configuration_home / FileKmpkgPathTxt, KMPKG_LINE_INFO);
    ...
    static void integrate_project(const KmpkgPaths& paths)
    {
#if defined(WIN32)
        auto& fs = paths.get_filesystem();
        ...
        const auto tmp_dir = fs.create_or_get_temp_directory(KMPKG_LINE_INFO);
        const auto targets_file_path = tmp_dir / "kmpkg.nuget.targets";
        const auto props_file_path = tmp_dir / "kmpkg.nuget.props";
        const auto nuspec_file_path = tmp_dir / "kmpkg.nuget.nuspec";
        const std::string nuget_id = get_nuget_id(paths.root);
        const std::string nupkg_version = "1.0.0";

        fs.write_contents(
            targets_file_path, create_nuget_targets_file_contents(paths.buildsystems_msbuild_targets), KMPKG_LINE_INFO);
        fs.write_contents(props_file_path, NUGET_PROPS_FILE_CONTENTS, KMPKG_LINE_INFO);
        fs.write_contents(
            nuspec_file_path, create_nuspec_file_contents(paths.root, nuget_id, nupkg_version), KMPKG_LINE_INFO);
        ...
        Checks::msg_exit_with_error(
            KMPKG_LINE_INFO, msgIntegrateWindowsOnly, msg::command_line = "kmpkg integrate project");
#endif // ^^^ !_WIN32
    }

    static void integrate_powershell(const KmpkgPaths& paths)
    {
#if defined(_WIN32)
        static constexpr StringLiteral TITLE = "PowerShell Tab-Completion";
        const auto script_path = paths.scripts / "addPoshKmpkgToPowershellProfile.ps1";
        ...
        Checks::exit_with_code(KMPKG_LINE_INFO, static_cast<int>(rc));
#else  // ^^^ _WIN32 // !_WIN32 vvv
        (void)paths;
        Checks::msg_exit_with_error(
            KMPKG_LINE_INFO, msgIntegrateWindowsOnly, msg::command_line = "kmpkg integrate powershell");
#endif // ^^^ !_WIN32
    }

    static void integrate_bash(const KmpkgPaths& paths)
    {
#if defined(_WIN32)
        (void)paths;
        Checks::msg_exit_with_error(
            KMPKG_LINE_INFO, msgIntegrateNonWindowsOnly, msg::command_line = "kmpkg integrate bash");
#else // ^^^ _WIN32 // !_WIN32 vvv
        const auto home_path = get_environment_variable("HOME").value_or_exit(KMPKG_LINE_INFO);
        ...
        auto& fs = paths.get_filesystem();
        const auto completion_script_path = paths.scripts / "kmpkg_completion.bash";

        auto bashrc_content = fs.read_contents(bashrc_path, KMPKG_LINE_INFO);
        auto matches = get_bash_source_completion_lines(bashrc_content);

        if (!matches.empty())
        {
            msg::println(msg::format(msgKmpkgCompletion, msg::value = "bash", msg::path = bashrc_path)
        ...
        fs.write_contents(bashrc_path, bashrc_content, KMPKG_LINE_INFO);
        Checks::exit_success(KMPKG_LINE_INFO);
#endif // ^^^ !_WIN32
    }

    static void integrate_zsh(const KmpkgPaths& paths)
    {
#if defined(_WIN32)
        (void)paths;
        Checks::msg_exit_with_error(
            KMPKG_LINE_INFO, msgIntegrateNonWindowsOnly, msg::command_line = "kmpkg integrate zsh");
#else  // ^^^ _WIN32 // !_WIN32 vvv
        const auto home_path = get_environment_variable("HOME").value_or_exit(KMPKG_LINE_INFO);
        const auto zshrc_path = Path{home_path} / ".zshrc";

        auto& fs = paths.get_filesystem();
        const auto completion_script_path = paths.scripts / "kmpkg_completion.zsh";

        auto zshrc_content = fs.read_contents(zshrc_path, KMPKG_LINE_INFO);
        ...
        {
            msg::println(msg::format(msgKmpkgCompletion, msg::value = "zsh", msg::path = zshrc_path)
        ...
        fs.write_contents(zshrc_path, zshrc_content, KMPKG_LINE_INFO);
        Checks::exit_success(KMPKG_LINE_INFO);
#endif // ^^^ !_WIN32
    }

    static void integrate_fish(const KmpkgPaths& paths)
    {
#if defined(_WIN32)
        (void)paths;
        Checks::msg_exit_with_error(
            KMPKG_LINE_INFO, msgIntegrateNonWindowsOnly, msg::command_line = "kmpkg integrate x-fish");
#else  // ^^^ _WIN32 // !_WIN32 vvv
        Path fish_completions_path;
        const auto config_path = get_environment_variable("XDG_CONFIG_HOME");
        if (config_path.has_value())
        {
            fish_completions_path = config_path.value_or_exit(KMPKG_LINE_INFO);
        }
        else
        {
            Path home_path = get_environment_variable("HOME").value_or_exit(KMPKG_LINE_INFO);
        ...
        fish_completions_path /= "kmpkg.fish";
        ...
        const auto completion_script_path = paths.scripts / "kmpkg_completion.fish";
        msg::println(msgAddingCompletionEntry, msg::path = fish_completions_path);
        fs.create_symlink(completion_script_path, fish_completions_path, KMPKG_LINE_INFO);
        Checks::exit_success(KMPKG_LINE_INFO);
#endif // ^^^ !_WIN32
    }
    ...
    static std::vector<std::string> valid_arguments(const KmpkgPaths&)
    {
...
#if defined(_WIN32)
            table.format("kmpkg integrate install", msg::format(msgIntegrateInstallHelpWindows));
#else  // ^^^ defined(_WIN32) // !defined(_WIN32) vvv
            table.format("kmpkg integrate install", msg::format(msgIntegrateInstallHelpLinux));
#endif // ^^^ !defined(_WIN32)
            table.format("kmpkg integrate remove", msg::format(msgIntegrateRemoveHelp));
            table.blank();
            table.format("kmpkg integrate project", msg::format(msgIntegrateProjectHelp));
            table.blank();
            table.format("kmpkg integrate bash", msg::format(msgIntegrateBashHelp));
            table.format("kmpkg integrate x-fish", msg::format(msgIntegrateFishHelp));
            table.format("kmpkg integrate powershell", msg::format(msgIntegratePowerShellHelp));
            table.format("kmpkg integrate zsh", msg::format(msgIntegrateZshHelp));
            return LocalizedString::from_raw("\n").append_raw(std::move(table.m_str));
        }},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/integrate",
    ...
    void command_integrate_and_exit(const KmpkgCmdArguments& args, const KmpkgPaths& paths)
    {
        ...
        Checks::msg_exit_with_message(
            KMPKG_LINE_INFO, msgUnknownParameterForIntegrate, msg::value = parsed.command_arguments[0]);
    }
}
