...
#include <kmpkg/base/xmlserializer.h>

#include <kmpkg/commands.export.h>
...
namespace
{
                                            ...
                                            const std::string& nuget_description)
    {
        ...
        xml.simple_tag("authors", "kmpkg").line_break();
        ...
        xml.start_complex_open_tag("file")
            .text_attr("src", raw_exported_dir.native() + "\\.kmpkg-root")
            .text_attr("target", "")
            .finish_self_closing_complex_tag()
            .line_break();
...
<Project ToolsVersion="4.0" xmlns="http://schemas.microsoft.com/developer/msbuild/2003">
  <Import Condition="Exists('{}')" Project="{}" />
    ...
    std::string create_export_id()
    {
        ...
        return ("kmpkg-export-" + date_time_as_string);
    }

    Path do_nuget_export(const KmpkgPaths& paths,
                         const std::string& nuget_id,
                         ...
                         const Path& output_dir)
    {
        ...
        const std::string targets_redirect_content =
            create_targets_redirect("$(MSBuildThisFileDirectory)../../scripts/buildsystems/msbuild/kmpkg.targets");
        const auto targets_redirect = paths.buildsystems / "tmp" / "kmpkg.export.nuget.targets";
        fs.write_contents(targets_redirect, targets_redirect_content, KMPKG_LINE_INFO);

        // This file will be placed in "build\native" in the nuget package. Therefore, go up two dirs.
        const std::string props_redirect_content =
            create_targets_redirect("$(MSBuildThisFileDirectory)../../scripts/buildsystems/msbuild/kmpkg.props");
        const auto props_redirect = paths.buildsystems / "tmp" / "kmpkg.export.nuget.props";
        fs.write_contents(props_redirect, props_redirect_content, KMPKG_LINE_INFO);
        ...
        const auto nuspec_file_path = paths.buildsystems / "tmp" / "kmpkg.export.nuspec";
        fs.write_contents(nuspec_file_path, nuspec_file_content, KMPKG_LINE_INFO);

        // -NoDefaultExcludes is needed for ".kmpkg-root"
        Command cmd;
#ifndef _WIN32
    ...
    Path do_archive_export(const KmpkgPaths& paths,
                           const Path& raw_exported_dir,
                           const Path& output_dir,
                           const ArchiveFormat& format)
    {
        ...
        Checks::msg_check_exit(
            KMPKG_LINE_INFO, succeeded(maybe_exit_code), msgCreationFailed, msg::path = exported_archive_path);
    ...
    ExportArguments handle_export_command_arguments(const KmpkgPaths& paths,
                                                    ...
                                                    const StatusParagraphs& status_db)
    {
        ...
        else
        {
            ...
            ret.specs = Util::fmap(options.command_arguments, [&](auto&& arg) {
                return parse_package_spec(arg, default_triplet).value_or_exit(KMPKG_LINE_INFO);
            });
        }
    ...
    {
        const auto cmake_toolchain = prefix / "scripts/buildsystems/kmpkg.cmake";
                                 ...
                                 const KmpkgPaths& paths)
    {
        ...
        fs.remove_all(raw_exported_dir_path, KMPKG_LINE_INFO);
        ...
        // execute the plan
        {
            ...
            for (const ExportPlanAction& action : export_plan)
            {
                ...
                const BinaryParagraph& binary_paragraph = action.core_paragraph().value_or_exit(KMPKG_LINE_INFO);
                const auto& triplet_canonical_name = action.spec.triplet().canonical_name();

                auto lines =
                    fs.read_lines(paths.installed().listfile_path(binary_paragraph)).value_or_exit(KMPKG_LINE_INFO);
        ...
        if (opts.nuget)
        {
            ...
            const auto nuget_description = opts.maybe_nuget_description.value_or("Kmpkg NuGet export");
...
namespace kmpkg
{
    void export_integration_files(const Path& raw_exported_dir_path, const KmpkgPaths& paths)
    {
        const std::vector<Path> integration_files_relative_to_root = {
            ...
            Path{"scripts/buildsystems/msbuild/kmpkg.targets"},
            Path{"scripts/buildsystems/msbuild/kmpkg.props"},
            Path{"scripts/buildsystems/msbuild/kmpkg-general.xml"},
            Path{"scripts/buildsystems/kmpkg.cmake"},
            Path{"scripts/buildsystems/osx/applocal.py"},
            Path{"scripts/cmake/kmpkg_get_windows_sdk.cmake"},
        };
        ...
        for (const Path& file : integration_files_relative_to_root)
        {
            ...
            fs.copy_file(source, destination, CopyOptions::overwrite_existing, KMPKG_LINE_INFO);
        }

        // Copying exe (this is not relative to root)
        Path kmpkg_exe = get_exe_path_of_current_process();
#if defined(_WIN32)
        auto destination = raw_exported_dir_path / "kmpkg.exe";
#else
        auto destination = raw_exported_dir_path / "kmpkg";
#endif
        fs.copy_file(kmpkg_exe, destination, CopyOptions::overwrite_existing, KMPKG_LINE_INFO);

        fs.write_contents(raw_exported_dir_path / ".kmpkg-root", "", KMPKG_LINE_INFO);
    }

    constexpr CommandMetadata CommandExportMetadata{
        "export",
        msgCmdExportSynopsis,
        {msgCmdExportExample1, "kmpkg export zlib zlib:x64-windows boost --nuget"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/export",
