...
namespace kmpkg
{
                        ...
                        StatusParagraphs& status_db)
    {
        ...
        Checks::msg_check_exit(KMPKG_LINE_INFO, maybe_ipv.has_value(), msgPackageAlreadyRemoved, msg::spec = spec);

        auto&& ipv = maybe_ipv.value_or_exit(KMPKG_LINE_INFO);
        ...
        if (auto lines = maybe_lines.get())
        {
            ...
            for (auto&& suffix : *lines)
            {
                ...
                else if (kmpkg::is_regular_file(status) || kmpkg::is_symlink(status))
                {
                    fs.remove(target, ec);
                    if (ec)
                    {
                ...
                else if (kmpkg::exists(status))
                {
                    Checks::unreachable(KMPKG_LINE_INFO, fmt::format("\"{}\": cannot handle file type", target));
                }
                else
                {
            ...
            fs.remove(installed.listfile_path(ipv.core->package), KMPKG_LINE_INFO);
        }
...
namespace
{
    ...
    std::vector<std::string> valid_arguments(const KmpkgPaths& paths)
    {
...
namespace kmpkg
{
    constexpr CommandMetadata CommandRemoveMetadata{
        ...
        {msgCmdRemoveExample1, "kmpkg remove zlib zlib:x64-windows curl boost", "kmpkg remove --outdated"},
        "https://pub.kumose.cc/kmdo/commands/kmpkg/remove",
                                 ...
                                 Triplet host_triplet)
    {
        ...
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgRemoveDependencies);
        }
        ...
        if (plan.empty())
        {
            Checks::unreachable(KMPKG_LINE_INFO, "Remove plan cannot be empty");
        }
