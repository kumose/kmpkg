...
#include <kmpkg/base/delayed-init.h>
...
#include <kmpkg/registries-parsing.h>
...
namespace
{
    ...
    {
        GitRegistry(const KmpkgPaths& paths, std::string&& repo, std::string&& reference, std::string&& baseline)
            : m_paths(paths)
    ...
    private:
        ...
        ExpectedL<VersionsTreePathResult> get_stale_versions_tree_path() const
        {
            ...
            return VersionsTreePathResult{m_stale_versions_tree.value_or_exit(KMPKG_LINE_INFO), true};
        }

        const KmpkgPaths& m_paths;
    ...
    {
        BuiltinGitRegistryEntry(const KmpkgPaths& paths) : m_paths(paths) { }
    ...
    {
        BuiltinFilesRegistry(const KmpkgPaths& paths)
    ...
    {
        BuiltinGitRegistry(const KmpkgPaths& paths, std::string&& baseline)
            : m_baseline_identifier(std::move(baseline))
    ...
    // read-only kmpkg if the user has not provided a baseline.
    ...
    static ExpectedL<Path> git_checkout_baseline(const KmpkgPaths& paths, StringView commit_sha)
    {
...
namespace kmpkg
{
    ...
    ExpectedL<Unit> LockFile::Entry::ensure_up_to_date(const KmpkgPaths& paths) const
    {
        ...
        : patterns_(std::move(patterns)), implementation_(std::move(impl))
    {
        ...
        Checks::check_exit(KMPKG_LINE_INFO, implementation_ != nullptr);
    }
...
namespace kmpkg
{
    ...
    ExpectedL<Baseline> get_builtin_baseline(const KmpkgPaths& paths)
    {
        return load_baseline_versions(paths.get_filesystem(), paths.builtin_registry_versions / FileBaselineDotJson);
    }

    std::unique_ptr<RegistryImplementation> make_builtin_registry(const KmpkgPaths& paths)
    {
    ...
    std::unique_ptr<RegistryImplementation> make_builtin_registry(const KmpkgPaths& paths, std::string baseline)
    {
        if (paths.use_git_default_registry())
        {
    ...
    std::unique_ptr<RegistryImplementation> make_git_registry(const KmpkgPaths& paths,
                                                              std::string repo,
    ...
    {
        static constexpr StringLiteral fields[] = {KMPKG_SCHEMED_DESERIALIZER_FIELDS, JsonIdPath};
        return fields;
    }
    ...
    {
        static constexpr StringLiteral fields[] = {KMPKG_SCHEMED_DESERIALIZER_FIELDS, JsonIdGitTree};
        return fields;
    }
