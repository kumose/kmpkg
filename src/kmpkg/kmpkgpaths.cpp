...
#include <kmpkg/binaryparagraph.h>
#include <kmpkg/bundlesettings.h>
...
namespace
{
    ...
    {
        return fs.almost_canonical(option ? Path(*option) : default_path, KMPKG_LINE_INFO);
    }

    ManifestAndPath load_manifest(const ReadOnlyFilesystem& fs, const Path& manifest_dir)
    {
        std::error_code ec;
        auto manifest_path = manifest_dir / "kmpkg.json";
                                ...
                                bool forbid_dot)
    {
        for (auto&& entry : overlay_entries)
        {
            if (forbid_dot && entry == ".")
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgErrorManifestMustDifferFromOverlayDot);
            }

            auto full_entry = relative_root / entry;
            if (forbid_dot && (fs.almost_canonical(full_entry, KMPKG_LINE_INFO) / "") == (config_directory / ""))
            {
                Checks::msg_exit_with_error(
                    KMPKG_LINE_INFO, msgErrorManifestMustDifferFromOverlay, msg::path = config_directory);
            }
                                                  ...
                                                  const KmpkgPaths& paths)
    {
        ...
        if (auto manifest = manifest_data.get())
        {
            if (auto config = manifest->config.get())
            {
                ...
                {
                    Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgBaselineConflict);
                }
                ...
                if (config_data.has_value())
                {
                    DiagnosticLine{DiagKind::Error,
                                   config_dir / FileKmpkgConfigurationDotJson,
                                   msg::format(msgAmbiguousConfig,
                                               msg::json_field = configuration_source_field(manifest->config_source))}
                        .print_to(stderr_sink);
                    DiagnosticLine{DiagKind::Note, manifest_dir / FileKmpkgDotJson, msg::format(msgManifestHere)}
                        .print_to(stderr_sink);
        ...
        if (auto config = config_data.get())
        {
            ...
            ret = ConfigurationAndSource{std::move(*config), config_dir, ConfigurationSource::KmpkgConfigurationFile};
        }
            ...
            if (!is_null_default && !has_baseline)
            {
                ...
                msg::write_unlocalized_text_to_stderr(Color::error,
                                                      msg::format(msgConfigurationErrorRegistriesWithoutBaseline,
                                                                  msg::path = origin,
                                                                  msg::url = kmpkg::docs::registries_url));
                Checks::exit_fail(KMPKG_LINE_INFO);
            }
        }
                                      ...
                                      LineInfo li)
    {
        ...
        else if (root_read_only)
        {
            ...
            {
                return fs.almost_canonical(i->kmpkg_dir() / readonly_subpath, li);
            }
            else
            {
                return nullopt;
    ...
    Path compute_manifest_dir(const ReadOnlyFilesystem& fs, const KmpkgCmdArguments& args, const Path& original_cwd)
    {
        ...
        {
            return fs.almost_canonical(*manifest_root_dir, KMPKG_LINE_INFO);
        }

        return fs.find_file_recursively_up(original_cwd, FileKmpkgDotJson, KMPKG_LINE_INFO);
    }

    // This structure holds members for KmpkgPathsImpl that don't require explicit initialization/destruction
    struct KmpkgPathsImplStage0
    {
        ...
        Optional<kmpkg::LockFile> m_installed_lock;
    };

    Path compute_registries_cache_root(const ReadOnlyFilesystem& fs, const KmpkgCmdArguments& args)
    {
        Path ret;
        if (auto registries_cache_dir = args.registries_cache_dir.get())
        {
            get_global_metrics_collector().track_define(DefineMetric::X_KmpkgRegistriesCache);
            ret = *registries_cache_dir;
            const auto status = real_filesystem.status(ret, KMPKG_LINE_INFO);

            if (!kmpkg::is_directory(status))
            {
                Checks::msg_exit_with_message(
                    KMPKG_LINE_INFO, msgKmpkgRegistriesCacheIsNotDirectory, msg::path = ret.native());
            }

            if (!ret.is_absolute())
            {
                Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgPathMustBeAbsolute, msg::path = ret.native());
            }
        }
        else
        {
            ret = get_platform_cache_kmpkg().value_or_exit(KMPKG_LINE_INFO) / "registries";
        }
    ...
    struct KmpkgPathsImplStage1 : KmpkgPathsImplStage0
    {
        KmpkgPathsImplStage1(const Filesystem& fs,
                             const KmpkgCmdArguments& args,
            ...
            , m_asset_cache_settings(
                  parse_download_configuration(args.asset_sources_template()).value_or_exit(KMPKG_LINE_INFO))
            ...
            , m_default_vs_path(args.default_visual_studio_path
                                    .map([&fs](const std::string& default_visual_studio_path) {
                                        return fs.almost_canonical(default_visual_studio_path, KMPKG_LINE_INFO);
                                    })
                                    .value_or(Path{}))
            , scripts(process_input_directory(fs, root, args.scripts_root_dir.get(), "scripts", KMPKG_LINE_INFO))
                                               ...
                                               const Path& manifest_dir)
    {
        ...
        else
        {
            return InstalledPaths{
                process_output_directory(fs, args.install_root_dir.get(), manifest_dir / "kmpkg_installed")};
        }
        return nullopt;
    }
                                ...
                                bool root_read_only)
    {
        ...
        {
            ret = get_platform_cache_kmpkg().value_or_exit(KMPKG_LINE_INFO) / "downloads";
        }
        else
        {
            ret = repo / "downloads";
    ...
    {
        Path ret;
        if (auto kmpkg_root_dir_arg = args.kmpkg_root_dir_arg.get())
        {
            ret = fs.almost_canonical(*kmpkg_root_dir_arg, KMPKG_LINE_INFO);
        }
        else
        {
            const auto canonical_current_exe = fs.almost_canonical(get_exe_path_of_current_process(), KMPKG_LINE_INFO);
            auto r = KmpkgConfig::get_root_from_config();
            if (r) {
                ret = *r;
            }

            if (auto kmpkg_root_dir_env = args.kmpkg_root_dir_env.get())
            {
                auto canonical_root_dir_env = fs.almost_canonical(*kmpkg_root_dir_env, KMPKG_LINE_INFO);
                if (ret.empty())
                {
                ...
                else if (ret != canonical_root_dir_env)
                {
                    msg::write_unlocalized_text_to_stderr(Color::warning,
                                                          LocalizedString::from_raw(WarningPrefix)
                                                              .append(msgIgnoringKmpkgRootEnvironment,
                                                                      msg::path = *kmpkg_root_dir_env,
                                                                      msg::actual = ret,
     ...
     Path determine_repo(const ReadOnlyFilesystem& fs, const Path& original_cwd, const KmpkgCmdArguments& args)
    {
        Path ret;
    ...
    {
#if defined(_WIN32)
        return kmpkg::win32_fix_path_case(fs.current_path(KMPKG_LINE_INFO));
#else
        return fs.current_path(KMPKG_LINE_INFO);
#endif
    }
    ...
    kmpkg::LockFile load_lockfile(const ReadOnlyFilesystem& fs, const Path& p)
    {
        kmpkg::LockFile ret;
        std::error_code ec;
...
namespace kmpkg
{
    ...
    {
        return this->kmpkg_dir_info() / (pgh.fullstem() + ".list");
    }

    struct KmpkgPathsImpl : KmpkgPathsImplStage1
    {
        KmpkgPathsImpl(const Filesystem& fs,
                       const KmpkgCmdArguments& args,
                       const BundleSettings bundle,
                        const Path& root,
                       const Path& repo,
                       const Path& original_cwd)
            : KmpkgPathsImplStage1(fs, args, bundle, root,repo, original_cwd)
            , m_global_config(bundle.read_only ? get_user_configuration_home().value_or_exit(KMPKG_LINE_INFO) /
                                                     "kmpkg-configuration.json"
                                               : root / "kmpkg-configuration.json")
            ...
            , m_tool_cache(get_tool_cache(
                  ...
                  m_asset_cache_settings,
                  downloads,
                  args.tools_data_file.has_value() ? Path{*args.tools_data_file.get()} : scripts / "kmpkg-tools.json",
                  tools,
            ...
            , m_artifacts_dir(downloads / "artifacts")
        {
            ...
            if (!m_manifest_dir.empty())
            {
                ...
                {
                    std::error_code ec;
                    const auto kmpkg_root_file = repo / ".kmpkg-root";
                    if (args.wait_for_lock.value_or(false))
                    {
                        file_lock_handle = fs.take_exclusive_file_lock(kmpkg_root_file, stderr_sink, ec);
                    }
                    else
                    {
                        file_lock_handle = fs.try_take_exclusive_file_lock(kmpkg_root_file, stderr_sink, ec);
                    }
                    ...
                    if (ec)
                    {
                        ...
                        if (is_already_locked || !allow_errors)
                        {
                            msg::write_unlocalized_text_to_stderr(
                                Color::error,
                                LocalizedString::from_raw(kmpkg_root_file)
                                    .append_raw(": ")
    ...
    KmpkgPaths::KmpkgPaths(const Filesystem& filesystem, const KmpkgCmdArguments& args, const BundleSettings& bundle)
        ...
        , m_pimpl(std::make_unique<KmpkgPathsImpl>(filesystem, args, bundle, root, repo, original_cwd))
        ...
        , buildsystems_msbuild_targets(buildsystems / "msbuild" / "kmpkg.targets")
        , buildsystems_msbuild_props(buildsystems / "msbuild" / "kmpkg.props")
        , ports_cmake(filesystem.almost_canonical(scripts / "ports.cmake", KMPKG_LINE_INFO))
        , triplets(filesystem.almost_canonical(root / "triplets", KMPKG_LINE_INFO))
        , community_triplets(filesystem.almost_canonical(triplets / "community", KMPKG_LINE_INFO))
    {
        Debug::print("Using kmpkg-root: ", root, '\n');
        ...
        const auto config_path = config_dir / "kmpkg-configuration.json";
        ...
        {
            m_pimpl->triplets_dirs.emplace_back(filesystem.almost_canonical(triplet, KMPKG_LINE_INFO));
        }
    ...
    KmpkgPaths::~KmpkgPaths() = default;

    Path KmpkgPaths::package_dir(const PackageSpec& spec) const { return this->packages() / spec.dir(); }
    Path KmpkgPaths::build_dir(const PackageSpec& spec) const { return this->buildtrees() / spec.name(); }
    Path KmpkgPaths::build_dir(StringView package_name) const { return this->buildtrees() / package_name.to_string(); }

    const TripletDatabase& KmpkgPaths::get_triplet_db() const
    {
        return m_pimpl->triplets_db.get_lazy([this]() -> TripletDatabase {
            ...
            {
                for (auto&& path : fs.get_regular_files_non_recursive(triplets_dir, KMPKG_LINE_INFO))
                {
    ...
    const std::map<std::string, std::string>& KmpkgPaths::get_cmake_script_hashes() const
    {
        return m_pimpl->cmake_script_hashes.get_lazy([this]() -> std::map<std::string, std::string> {
            ...
            auto files = fs.get_regular_files_non_recursive(this->scripts / "cmake", KMPKG_LINE_INFO);
            for (auto&& file : files)
            {
                ...
                helpers.emplace(file.stem().to_string(),
                                Hash::get_file_hash(fs, file, Hash::Algorithm::Sha256).value_or_exit(KMPKG_LINE_INFO));
            }
            return helpers;
        });
    }

    StringView KmpkgPaths::get_ports_cmake_hash() const
    {
    ...
    LockFile& KmpkgPaths::get_installed_lockfile() const
    {
    ...
    void KmpkgPaths::flush_lockfile() const
    {
        ...
        get_filesystem().write_rename_contents(
            installed().lockfile_path(), "kmpkg-lock.json.tmp", Json::stringify(obj), KMPKG_LINE_INFO);
    }
    const Optional<InstalledPaths>& KmpkgPaths::maybe_installed() const { return m_pimpl->m_installed; }
    const Optional<Path>& KmpkgPaths::maybe_buildtrees() const { return m_pimpl->buildtrees; }
    const Optional<Path>& KmpkgPaths::maybe_packages() const { return m_pimpl->packages; }

    const Path& KmpkgPaths::global_config() const { return m_pimpl->m_global_config; }

    const InstalledPaths& KmpkgPaths::installed() const
    {
        ...
        Checks::msg_exit_with_error(KMPKG_LINE_INFO,
                                    msg::format(msgKmpkgDisallowedClassicMode)
                                        .append_raw('\n')
                                        .append(msgSeeURL, msg::url = docs::troubleshoot_build_failures_url));
    }

    const Path& KmpkgPaths::buildtrees() const
    {
    ...
    const Path& KmpkgPaths::packages() const
    {
    ...
    Path KmpkgPaths::baselines_output() const { return buildtrees() / "versioning_" / "baselines"; }
    Path KmpkgPaths::versions_output() const { return buildtrees() / "versioning_" / "versions"; }

    ExpectedL<Path> KmpkgPaths::versions_dot_git_dir() const
    {
    ...
    std::string KmpkgPaths::get_toolver_diagnostics() const
    {
        std::string ret;
        Strings::append(ret, "    kmpkg-tool version: ", kmpkg_executable_version, "\n");
        if (m_pimpl->m_bundle.read_only)
        {
            Strings::append(ret, "    kmpkg-readonly: true\n");
            const auto sha = get_current_git_sha();
            Strings::append(ret, "    kmpkg-scripts version: ", sha ? StringView(*sha.get()) : "unknown", "\n");
        }
        else
        {
            ...
            {
                Strings::append(ret, "    kmpkg-scripts version: ", *output, "\n");
            }
            else
            {
                Strings::append(ret, "    kmpkg-scripts version: unknown\n");
            }
        }
        return ret;
    }

    const Filesystem& KmpkgPaths::get_filesystem() const { return m_pimpl->m_fs; }
    const AssetCachingSettings& KmpkgPaths::get_asset_cache_settings() const { return m_pimpl->m_asset_cache_settings; }
    const ToolCache& KmpkgPaths::get_tool_cache() const { return *m_pimpl->m_tool_cache; }
    const Path& KmpkgPaths::get_tool_exe(StringView tool, MessageSink& status_messages) const
    {
        return m_pimpl->m_tool_cache->get_tool_path(tool, status_messages);
    }
    const std::string& KmpkgPaths::get_tool_version(StringView tool, MessageSink& status_messages) const
    {
        return m_pimpl->m_tool_cache->get_tool_version(tool, status_messages);
    }

    Command KmpkgPaths::git_cmd_builder(const Path& dot_git_dir, const Path& work_tree) const
    {
    ...
    ExpectedL<std::string> KmpkgPaths::get_current_git_sha() const
    {
    ...
    LocalizedString KmpkgPaths::get_current_git_sha_baseline_message() const
    {
    ...
    ExpectedL<Path> KmpkgPaths::git_checkout_port(StringView port_name,
                                                  StringView git_tree,
    ...
    ExpectedL<std::string> KmpkgPaths::git_show(StringView treeish, const Path& dot_git_dir) const
    {
        // All git commands are run with: --git-dir={dot_git_dir} --work-tree={work_tree_temp}
        // git clone --no-checkout --local {kmpkg_root} {dot_git_dir}
    ...
    Optional<std::vector<GitLSTreeEntry>> KmpkgPaths::get_builtin_ports_directory_trees(
        ...
        if (auto prefix = maybe_prefix.get())
        {
            ...
            if (const auto index_file = maybe_index_file.get())
            {
                TempFileDeleter temp_index_file{fs,
                                                fmt::format("{}_kmpkg_{}.tmp", index_file->native(), get_process_id())};
    ...
    ExpectedL<std::string> KmpkgPaths::git_fetch_from_remote_registry(StringView repo, StringView treeish) const
    {
        auto& fs = get_filesystem();

        const auto& work_tree = m_pimpl->m_registries_work_tree_dir;
        fs.create_directories(work_tree, KMPKG_LINE_INFO);
        ...
        auto lock_file = work_tree / ".kmpkg-lock";
    ...
    ExpectedL<Unit> KmpkgPaths::git_fetch(StringView repo, StringView treeish) const
    {
        auto& fs = get_filesystem();
    ...
    // hash
    ExpectedL<std::string> KmpkgPaths::git_show_from_remote_registry(StringView hash, const Path& relative_path) const
    {
    ...
    ExpectedL<std::string> KmpkgPaths::git_find_object_id_for_remote_registry_path(StringView hash,
    ...
    ExpectedL<Unit> KmpkgPaths::git_read_tree(const Path& destination, StringView tree, const Path& dot_git_dir) const
    {
        BufferedDiagnosticContext bdc{out_sink};
        if (kmpkg::git_extract_tree(bdc,
                                    get_filesystem(),
                                    get_tool_exe(Tools::GIT, out_sink),
    ...
    ExpectedL<Path> KmpkgPaths::git_extract_tree_from_remote_registry(StringView tree) const
    {
    ...
    Optional<const ManifestAndPath&> KmpkgPaths::get_manifest() const
    {
    ...
    bool KmpkgPaths::manifest_mode_enabled() const { return m_pimpl->m_manifest_doc.has_value(); }

    const ConfigurationAndSource& KmpkgPaths::get_configuration() const { return m_pimpl->m_config; }

    std::unique_ptr<RegistrySet> KmpkgPaths::make_registry_set() const
    {
...
#if defined(_WIN32)
    static const ToolsetsInformation& get_all_toolsets(KmpkgPathsImpl& impl, const ReadOnlyFilesystem& fs)
    {
    ...
    const Toolset& KmpkgPaths::get_toolset(const PreBuildInfo& prebuildinfo) const
    {
...
#if !defined(WIN32)
        Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgErrorVcvarsUnsupported, msg::triplet = prebuildinfo.triplet);
#else
        ...
        if (candidate == vs_toolsets.end())
        {
            ...
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, error_message);
        }
        return *candidate;
#endif
    }

    const Environment& KmpkgPaths::get_action_env(const PreBuildInfo& pre_build_info, const Toolset& toolset) const
    {
        return m_pimpl->m_env_cache.get_action_env(*this, pre_build_info, toolset);
    }

    const std::string& KmpkgPaths::get_triplet_info(const PreBuildInfo& pre_build_info, const Toolset& toolset) const
    {
        return m_pimpl->m_env_cache.get_triplet_info(*this, pre_build_info, toolset);
    }

    const CompilerInfo& KmpkgPaths::get_compiler_info(const PreBuildInfo& pre_build_info, const Toolset& toolset) const
    {
        return m_pimpl->m_env_cache.get_compiler_info(*this, pre_build_info, toolset);
    }

    const FeatureFlagSettings& KmpkgPaths::get_feature_flags() const { return m_pimpl->m_ff_settings; }

    const Path& KmpkgPaths::builtin_ports_directory() const { return m_pimpl->m_builtin_ports; }

    bool KmpkgPaths::use_git_default_registry() const { return m_pimpl->m_bundle.use_git_registry; }

    const Path& KmpkgPaths::artifacts() const { return m_pimpl->m_artifacts_dir; }
    const Path& KmpkgPaths::registries_cache() const { return m_pimpl->m_registries_cache; }
} // namespace kmpkg
