...
#include <kmpkg/base/system.deviceid.h>
#include <kmpkg/base/system.h>
#include <kmpkg/base/system.mac.h>
...
namespace kmpkg
{
    ...
    const constexpr DefineMetricEntry all_define_metrics[static_cast<size_t>(DefineMetric::COUNT)] = {
        ...
        {DefineMetric::KmpkgBinarySources, "KMPKG_BINARY_SOURCES"},
        {DefineMetric::KmpkgDefaultBinaryCache, "KMPKG_DEFAULT_BINARY_CACHE"},
        {DefineMetric::KmpkgNuGetRepository, "KMPKG_NUGET_REPOSITORY"},
        ...
        {DefineMetric::X_KmpkgRegistriesCache, "X_KMPKG_REGISTRIES_CACHE"},
    ...
    const constexpr StringMetricEntry all_string_metrics[static_cast<size_t>(StringMetric::COUNT)] = {
        ...
        {StringMetric::KmpkgVersion, "kmpkg_version", "2991-12-31-unknownhash"},
    ...
    void winhttp_upload_metrics(StringView payload)
    {
        ...
        const HINTERNET session = WinHttpOpen(
            L"kmpkg/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    ...
    void flush_global_metrics(const Filesystem& fs)
    {
        ...
        const Path temp_folder_path = fs.create_or_get_temp_directory(KMPKG_LINE_INFO);
        const Path kmpkg_metrics_txt_path = temp_folder_path / ("kmpkg" + generate_random_UUID() + ".txt");
        Debug::println("Uploading metrics ", kmpkg_metrics_txt_path);
        std::error_code ec;
        fs.write_contents(kmpkg_metrics_txt_path, payload, ec);
        if (ec) return;

#if defined(_WIN32)
        const Path temp_folder_path_exe = temp_folder_path / "kmpkg-" KMPKG_BASE_VERSION_AS_STRING ".exe";
        ...
        builder.string_arg(kmpkg_metrics_txt_path);
        ...
        cmd_execute_background(Command("curl")
                                   ...
                                   .string_arg("--tlsv1.2")
                                   .string_arg("--data")
                                   .string_arg(Strings::concat("@", kmpkg_metrics_txt_path))
                                   .raw_arg(">/dev/null")
                                   .raw_arg("2>&1")
                                   .raw_arg(";")
                                   .string_arg("rm")
                                   .string_arg(kmpkg_metrics_txt_path));
#endif
    }
}
