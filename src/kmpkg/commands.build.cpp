...
namespace
{
    ...
    {
        void record_build_result(const KmpkgPaths& paths, const PackageSpec& spec, BuildResult result) const override
        {
            (void)paths;
            (void)spec;
            (void)result;
...
namespace kmpkg
{
    ...
    void CiBuildLogsRecorder::record_build_result(const KmpkgPaths& paths,
                                                  ...
                                                  BuildResult result) const
    {
        ...
        if (minimum_last_write_time > 0)
        {
            Util::erase_remove_if(children, [&](Path& path) {
                return filesystem.last_write_time(path, KMPKG_LINE_INFO) < minimum_last_write_time;
            });
        }
        auto target_path = base_path / spec.name();
        (void)filesystem.create_directories(target_path, KMPKG_LINE_INFO);
        if (children.empty())
        {
            auto message =
                fmt::format("There are no build logs for {} build.\n"
                            ...
                            "See the console output logs from kmpkg for more information on the failure.\n",
                            spec);
            filesystem.write_contents(target_path / FileReadmeDotLog, message, KMPKG_LINE_INFO);
        }
        else
        {
            for (const Path& p : children)
            {
                filesystem.copy_file(p, target_path / p.filename(), CopyOptions::overwrite_existing, KMPKG_LINE_INFO);
            }
        }
    }
    ...
    void purge_packages_dirs(const KmpkgPaths& paths, View<std::string> spec_dirs)
    {
        auto& fs = paths.get_filesystem();
        for (const auto& package_dir : fs.get_directories_non_recursive(paths.packages(), KMPKG_LINE_INFO))
        {
            ...
            {
                fs.remove_all(package_dir, KMPKG_LINE_INFO);
            }
        }
    }

    void command_build_and_exit_ex(const KmpkgCmdArguments& args,
    ...
    constexpr CommandMetadata CommandBuildMetadata{
        "build",
        msgCmdBuildSynopsis,
        {msgCmdBuildExample1, "kmpkg build zlib:x64-windows"},
        Undocumented,
    ...
    void command_build_and_exit(const KmpkgCmdArguments& args,
                                const KmpkgPaths& paths,
    ...
    int command_build_ex(const KmpkgCmdArguments& args,
                         const KmpkgPaths& paths,
                         Triplet host_triplet,
                         const BuildPackageOptions& build_options,
                         const FullPackageSpec& full_spec,
                         ...
                         const IBuildLogsRecorder& build_logs_recorder)
    {
        ...
        for (auto& install_action : action_plan.already_installed)
        {
            ...
            {
                Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgBuildAlreadyInstalled, msg::spec = spec);
            }
        }
        ...
        Checks::check_exit(KMPKG_LINE_INFO, action != nullptr);
        ASSUME(action != nullptr);
        auto& scf = *action->source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).source_control_file;
        ...
        switch (result.code)
        {
            ...
            case BuildResult::CascadedDueToMissingDependencies:
            {
                ...
                Checks::msg_exit_with_message(KMPKG_LINE_INFO, errorMsg);
            }
    ...
    static ZStringView to_vcvarsall_target(StringView cmake_system_name)
    {
        ...
        Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgUnsupportedSystemName, msg::system_name = cmake_system_name);
    }
    ...
    static ZStringView to_vcvarsall_toolchain(StringView target_architecture, const Toolset& toolset, Triplet triplet)
    {
        ...
        auto target_arch = maybe_target_arch.value_or_exit(KMPKG_LINE_INFO);
        ...
        msg::println(msgSeeURL, msg::url = docs::kmpkg_visual_studio_path_url);
        Checks::exit_fail(KMPKG_LINE_INFO);
    }
#endif

#if defined(_WIN32)
    const Environment& EnvCache::get_action_env(const KmpkgPaths& paths,
                                                ...
                                                const Toolset& toolset)
    {
        ...
        const auto& base_env = envs.get_lazy(pre_build_info.passthrough_env_vars, [&]() -> EnvMapEntry {
            ...
            static constexpr StringLiteral s_extra_vars[] = {
                EnvironmentVariableKmpkgCommand,
                EnvironmentVariableKmpkgForceSystemBinaries,
                EnvironmentVariableXKmpkgRecursiveData,
            };
             ...
             * Settings, but not setting environment variables. This will make kmpkg easier to use, specially when
             * use kmpkg in Visual Studio, we even cannot set HTTP(S)_PROXY in CLI, if we want to open or close
...
#else
    const Environment& EnvCache::get_action_env(const KmpkgPaths&, const PreBuildInfo&, const Toolset&)
    {
        return get_clean_environment();
    }
#endif

    static CompilerInfo load_compiler_info(const KmpkgPaths& paths,
                                           const PreBuildInfo& pre_build_info,
                                           const Toolset& toolset);
                                                  ...
                                                  const ReadOnlyFilesystem& fs)
    {
        return cache.get_lazy(tcfile, [&]() {
            return Hash::get_file_hash(fs, tcfile, Hash::Algorithm::Sha256).value_or_exit(KMPKG_LINE_INFO);
        });
    }
    ...
    const EnvCache::TripletMapEntry& EnvCache::get_triplet_cache(const ReadOnlyFilesystem& fs, const Path& p) const
    {
        return m_triplet_cache.get_lazy(p, [&]() -> TripletMapEntry {
            return TripletMapEntry{Hash::get_file_hash(fs, p, Hash::Algorithm::Sha256).value_or_exit(KMPKG_LINE_INFO)};
        });
    }

    const CompilerInfo& EnvCache::get_compiler_info(const KmpkgPaths& paths,
                                                    const PreBuildInfo& pre_build_info,
                                                    const Toolset& toolset)
    {
    ...
    const std::string& EnvCache::get_triplet_info(const KmpkgPaths& paths,
                                                  const PreBuildInfo& pre_build_info,
                                                  const Toolset& toolset)
    {
        const auto& fs = paths.get_filesystem();
    ...
    kmpkg::Command make_build_env_cmd(const PreBuildInfo& pre_build_info, const Toolset& toolset)
    {
        ...
        return kmpkg::Command{"cmd"}.string_arg("/d").string_arg("/c").raw_arg(
    ...
    {
        const auto& scfl = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO);

        auto bcf = std::make_unique<BinaryControlFile>();

        auto find_itr = action.feature_dependencies.find(FeatureNameCore.to_string());
        Checks::check_exit(KMPKG_LINE_INFO, find_itr != action.feature_dependencies.end());
        BinaryParagraph bpgh(*scfl.source_control_file->core_paragraph,
                             action.default_features.value_or_exit(KMPKG_LINE_INFO),
                             action.spec.triplet(),
                             action.public_abi(),
    ...
    static void write_binary_control_file(const Filesystem& fs, const Path& package_dir, const BinaryControlFile& bcf)
    {
        ...
        fs.write_contents(binary_control_file, start, KMPKG_LINE_INFO);
    }

    static void get_generic_cmake_build_args(const KmpkgPaths& paths,
                                             Triplet triplet,
                                             const Toolset& toolset,
                                             std::vector<CMakeVariable>& out_vars)
    {
        ...
        out_vars.emplace_back(CMakeVariableBaseVersion, KMPKG_BASE_VERSION_AS_STRING);
                                           ...
                                           const Toolset& toolset)
    {
        ...
        auto cmd = kmpkg::make_cmake_cmd(paths, paths.ports_cmake, std::move(cmake_args));
        RedirectedProcessLaunchSettings settings;
        settings.environment.emplace(paths.get_action_env(pre_build_info, toolset));
        auto& fs = paths.get_filesystem();
        fs.create_directory(buildpath, KMPKG_LINE_INFO);
        auto stdoutlog = buildpath / ("stdout-" + triplet.canonical_name() + ".log");
        CompilerInfo compiler_info;
        std::string buf;

        Optional<WriteFilePointer> out_file_storage = fs.open_for_write(stdoutlog, KMPKG_LINE_INFO);
        auto& out_file = out_file_storage.value_or_exit(KMPKG_LINE_INFO);
        ...
        if (compiler_info.hash.empty() || !succeeded(rc))
        {
            ...
            Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgErrorUnableToDetectCompilerInfo);
        }
    ...
    static std::vector<CMakeVariable> get_cmake_build_args(const KmpkgCmdArguments& args,
                                                           const KmpkgPaths& paths,
                                                           Triplet host_triplet,
                                                           const BuildPackageOptions& build_options,
                                                           const InstallPlanAction& action)
    {
        auto& scfl = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO);
        ...
        auto& post_portfile_includes = action.pre_build_info(KMPKG_LINE_INFO).post_portfile_includes;
        ...
        std::vector<CMakeVariable> variables{
            ...
            {CMakeVariableCurrentPackagesDir, action.package_dir.value_or_exit(KMPKG_LINE_INFO)},
            ...
            {CMakeVariableZChainloadToolchainFile, action.pre_build_info(KMPKG_LINE_INFO).toolchain_file()},
        ...
        if (auto cmake_configure_debug = args.cmake_configure_debug.get())
        {
            ...
            {
                variables.emplace_back(fmt::format("-DKMPKG_CMAKE_CONFIGURE_OPTIONS=--debugger;--debugger-pipe={}",
        ...
        get_generic_cmake_build_args(
            paths,
            action.spec.triplet(),
            action.abi_info.value_or_exit(KMPKG_LINE_INFO).toolset.value_or_exit(KMPKG_LINE_INFO),
            variables);
        ...
        {
            Path port_config_path = paths.installed().kmpkg_port_config_cmake(dependency);
    ...
    static void write_sbom(const KmpkgPaths& paths,
                           const InstallPlanAction& action,
                           std::vector<Json::Object> heuristic_resources)
    {
        ...
        const auto& abi = action.abi_info.value_or_exit(KMPKG_LINE_INFO);
        const auto& package_dir = action.package_dir.value_or_exit(KMPKG_LINE_INFO);

        const auto json_path = package_dir / FileShare / action.spec.name() / FileKmpkgSpdxJson;
    ...
    static ExtendedBuildResult do_build_package(const KmpkgCmdArguments& args,
                                                const KmpkgPaths& paths,
                                                Triplet host_triplet,
                                                const BuildPackageOptions& build_options,
                                                const InstallPlanAction& action,
    ...
    {
        const auto& pre_build_info = action.pre_build_info(KMPKG_LINE_INFO);
        ...
        const auto& abi_info = action.abi_info.value_or_exit(KMPKG_LINE_INFO);

        const ElapsedTimer timer;
        auto cmd = kmpkg::make_cmake_cmd(
        ...
        auto& env = settings.environment.emplace(
            paths.get_action_env(*abi_info.pre_build_info, abi_info.toolset.value_or_exit(KMPKG_LINE_INFO)));

        auto buildpath = paths.build_dir(action.spec);
        fs.create_directory(buildpath, KMPKG_LINE_INFO);
        env.add_entry(EnvironmentVariableGitCeilingDirectories, fs.absolute(buildpath.parent_path(), KMPKG_LINE_INFO));
        ...
        if (build_failed)
        {
            ...
            if (fs.exists(logs, KMPKG_LINE_INFO))
            {
                error_logs = fs.read_lines(logs).value_or_exit(KMPKG_LINE_INFO);
        ...
        const BuildInfo build_info =
            read_build_info(fs, action.package_dir.value_or_exit(KMPKG_LINE_INFO) / FileBuildInfo);
        ...
        write_binary_control_file(paths.get_filesystem(), action.package_dir.value_or_exit(KMPKG_LINE_INFO), *bcf);
        return {BuildResult::Succeeded, std::move(bcf)};
    }

    static ExtendedBuildResult do_build_package_and_clean_buildtrees(const KmpkgCmdArguments& args,
                                                                     const KmpkgPaths& paths,
                                                                     Triplet host_triplet,
                                                                     const BuildPackageOptions& build_options,
                                                                     const InstallPlanAction& action,
                                                ...
                                                std::vector<AbiEntry>& abi_tag_entries)
    {
        if (pre_build_info.public_abi_override)
        {
            abi_tag_entries.emplace_back(
                ...
                Hash::get_string_hash(pre_build_info.public_abi_override.value_or_exit(KMPKG_LINE_INFO),
        ...
        for (const auto& env_var : pre_build_info.passthrough_env_vars_tracked)
        {
            ...
            {
                abi_tag_entries.emplace_back(
                    "ENV:" + env_var, Hash::get_string_hash(e.value_or_exit(KMPKG_LINE_INFO), Hash::Algorithm::Sha256));
            }
        }
    ...
    static void populate_abi_tag(const KmpkgPaths& paths,
                                 ...
                                 Cache<Path, Optional<std::string>>& grdk_cache)
    {
        Checks::check_exit(KMPKG_LINE_INFO, static_cast<bool>(proto_pre_build_info));
        ...
        auto&& port_dir = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).port_directory();
        const auto& port_dir_cache_entry = port_dir_cache.get_lazy(port_dir, [&]() {
            ...
            {
                auto rel_port_files = fs.get_regular_files_recursive_lexically_proximate(port_dir, KMPKG_LINE_INFO);
            ...
            for (size_t i = 0; i < abi_info.pre_build_info->hash_additional_files.size(); ++i)
            {
                ...
                if (file.is_relative() || !fs.is_regular_file(file))
                {
                    Checks::msg_exit_with_message(
                        KMPKG_LINE_INFO, msgInvalidValueHashAdditionalFiles, msg::path = file);
                }
                abi_tag_entries.emplace_back(
            ...
            for (const Path& rel_port_file : rel_port_files)
            {
                ...
                {
                    const auto contents = fs.read_contents(abs_port_file, KMPKG_LINE_INFO);
                    portfile_cmake_contents += contents;
                    port_dir_cache_entry.hashes.push_back(kmpkg::Hash::get_string_sha256(contents));
                }
                else
                {
                    port_dir_cache_entry.hashes.push_back(
                        kmpkg::Hash::get_file_hash(fs, abs_port_file, Hash::Algorithm::Sha256)
                            .value_or_exit(KMPKG_LINE_INFO));
                }
...

        {
            ...
            for (auto& filestr : abi_info.pre_build_info->hash_additional_files)
            {
                ...
                const auto hash =
                    kmpkg::Hash::get_file_hash(fs, file, Hash::Algorithm::Sha256).value_or_exit(KMPKG_LINE_INFO);
        ...
        for (size_t i = 0; i < abi_info.pre_build_info->post_portfile_includes.size(); ++i)
        {
            ...
            {
                Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgInvalidValuePostPortfileIncludes, msg::path = file);
            }
        ...
        Checks::check_exit(KMPKG_LINE_INFO, has_no_pseudo_features);
        ...
        Path abi_file_path = paths.build_dir(action.spec) / (triplet_canonical_name + ".kmpkg_abi_info.txt");
        fs.write_contents_and_dirs(abi_file_path, full_abi_info, KMPKG_LINE_INFO);
    ...
    void compute_all_abis(const KmpkgPaths& paths,
        ...
        for (auto it = action_plan.install_actions.begin(); it != action_plan.install_actions.end(); ++it)
        {
            ...
            populate_abi_tag(
                ...
                std::make_unique<PreBuildInfo>(paths,
                                               action.spec.triplet(),
                                               var_provider.get_tag_vars(action.spec).value_or_exit(KMPKG_LINE_INFO)),
                dependency_abis,
                port_dir_cache,
                grdk_cache);
        }
    ...
    ExtendedBuildResult build_package(const KmpkgCmdArguments& args,
                                      const KmpkgPaths& paths,
                                      Triplet host_triplet,
                                      const BuildPackageOptions& build_options,
                                      const InstallPlanAction& action,
                                      ...
                                      const StatusParagraphs& status_db)
    {
        ...
        const std::string& name = action.source_control_file_and_location.value_or_exit(KMPKG_LINE_INFO).to_name();
        ...
        if (build_options.only_downloads == OnlyDownloads::No)
        {
            ...
            for (auto&& pspec : action.package_dependencies)
            {
                ...
                {
                    Checks::msg_exit_with_error(KMPKG_LINE_INFO, msgCorruptedDatabase);
                }
            }
        }

        auto& abi_info = action.abi_info.value_or_exit(KMPKG_LINE_INFO);
        ...
        if (abi_info.abi_tag_file)
        {
            ...
            const auto abi_package_dir = action.package_dir.value_or_exit(KMPKG_LINE_INFO) / FileShare / spec.name();
            const auto abi_file_in_package = abi_package_dir / FileKmpkgAbiInfo;
            build_logs_recorder.record_build_result(paths, spec, result.code);
            filesystem.create_directories(abi_package_dir, KMPKG_LINE_INFO);
            filesystem.copy_file(abi_file, abi_file_in_package, CopyOptions::none, KMPKG_LINE_INFO);
        }
    ...
    std::string create_github_issue(const KmpkgCmdArguments& args,
                                    ...
                                    bool include_manifest)
    {
        ...
        fmt::format_to(std::back_inserter(issue_body),
                       "**Failure logs**\n\n```\n{}\n```\n\n",
                       paths.get_filesystem().read_contents(build_result.stdoutlog.value_or_exit(KMPKG_LINE_INFO),
                                                            KMPKG_LINE_INFO));
        ...
        if (auto manifest = maybe_manifest.get())
        {
            if (include_manifest || manifest->manifest.contains("builtin-baseline"))
            {
                fmt::format_to(
                    ...
                    "**Additional context**\n\n<details><summary>kmpkg.json</summary>\n\n```\n{}\n```\n</details>\n",
        ...
        if (issue_body.size() + postfix.size() < MAX_ISSUE_SIZE)
        {
            ...
            auto logs = Util::fmap(build_result.error_logs, [&](auto&& path) -> std::pair<Path, std::string> {
                return {path, fs.read_contents(path, KMPKG_LINE_INFO)};
            });
    ...
    {
        return "https://github.com/kumose/kmpkg/issues?q=is%3Aissue+is%3Aopen+in%3Atitle+" + spec_name;
    }
    ...
    static std::string make_gh_issue_open_url(StringView spec_name, StringView triplet, StringView body)
    {
        ...
        return Strings::concat("https://github.com/kumose/kmpkg/issues/new?title=",
                                        ...
                                        const Path& file)
    {
        ...
        auto contents = fs.read_contents(file, KMPKG_LINE_INFO);
        switch (kind)
        {
            case CIKind::GithubActions:
            ...
            case CIKind::TeamCityCI:
            case CIKind::TravisCI:
            case CIKind::Generic: Checks::unreachable(KMPKG_LINE_INFO, "CIKind not collapsible");
            default: Checks::unreachable(KMPKG_LINE_INFO);
        }
    }
                                                        ...
                                                        const Optional<Path>& maybe_issue_body)
    {
        ...
        if (auto issue_body = maybe_issue_body.get())
        {
            ...
            // rather than instructions for the current user of kmpkg.
            ...
            else
            {
                ...
                if (!gh_path.empty())
                {
                    ...
                    gh.string_arg("issue").string_arg("create").string_arg("-R").string_arg("kumose/kmpkg");
        ...
        {
            result
                .append_raw("https://github.com/kumose/kmpkg/issues/"
    ...
    static BuildInfo inner_create_buildinfo(StringView origin, Paragraph&& pgh)
    {
...

        {
            ...
            {
                Checks::msg_exit_with_message(
                    KMPKG_LINE_INFO, msgInvalidLinkage, msg::system_name = "crt", msg::value = crt_linkage_as_string);
            }
        }
        ...
        if (!version.empty())
        {
            ...
            build_info.detected_head_version = Version::parse(std::move(version)).value_or_exit(KMPKG_LINE_INFO);
        }
        ...
        {
            Checks::msg_exit_with_message(KMPKG_LINE_INFO, *err);
        }
    ...
    BuildInfo read_build_info(const ReadOnlyFilesystem& fs, const Path& filepath)
    {
        ...
        Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgInvalidBuildInfo, msg::error_msg = maybe_paragraph.error());
    }
    ...
    PreBuildInfo::PreBuildInfo(const KmpkgPaths& paths,
                               Triplet triplet,
        ...
        : triplet(triplet), m_paths(paths)
    {
        ...
        if (auto value = Util::value_if_set_and_nonempty(cmakevars, CMakeVariableBuildType))
        {
            if (Strings::case_insensitive_ascii_equals(*value, "debug"))
                ...
                Checks::msg_exit_with_message(KMPKG_LINE_INFO, msgUnknownSettingForBuildType, msg::option = *value);
        }
        ...
        {
            load_vcvars_env = from_cmake_bool(*value, CMakeVariableLoadVcvarsEnv).value_or_exit(KMPKG_LINE_INFO);
        }

        if (auto value = Util::value_if_set_and_nonempty(cmakevars, CMakeVariableDisableCompilerTracking))
        {
            disable_compiler_tracking =
                from_cmake_bool(*value, CMakeVariableDisableCompilerTracking).value_or_exit(KMPKG_LINE_INFO);
        }
        ...
        Util::assign_if_set_and_nonempty(gamedk_latest_path, cmakevars, CMakeVariableZKmpkgGameDKLatest);
        Util::assign_if_set_and_nonempty(gamedk_xbox_latest_path, cmakevars, CMakeVariableZKmpkgGameDKXboxLatest);
    }

    ExtendedBuildResult::ExtendedBuildResult(BuildResult code) : code(code) { }
    ExtendedBuildResult::ExtendedBuildResult(BuildResult code,
                                             kmpkg::Path stdoutlog,
                                             std::vector<std::string>&& error_logs)
