...
TEST_CASE ("specifier parsing", "[specifier]")
{
    ...
    {
        auto spec = kmpkg::parse_qualified_specifier(
    ...
    SECTION ("parsed specifier from string with triplet")
    {
        ...
        REQUIRE(spec.name.value == "zlib");
        REQUIRE(!spec.features);
        REQUIRE(spec.triplet.value_or_exit(KMPKG_LINE_INFO).value == "x64-uwp");
        REQUIRE(!spec.platform);
    ...
    {
        auto s = kmpkg::parse_qualified_specifier(
        ...
        else
        {
            REQUIRE(
                ...
                LocalizedString::from_raw(
                    ...
                    docs::kmpkg_json_ref_name + R"( for more information).
  on expression: zlib#
                     ^)"));
        }
    }
  ...
  on expression: zlib:x86-uwp (windows)[co,  re]
    ...
    SECTION ("parsed specifier from string with unclosed feature suffix")
    {
        ...
        // should not engage
        auto s = kmpkg::parse_qualified_specifier("zlib:x64-windows[no-ending-square-bracket",
                                                  AllowFeatures::Yes,
  ...
  on expression: zlib:x64-windows[feature]suffix
    ...
    SECTION ("parsed specifier from string with feature")
    {
        ...
        auto spec_forbidding_features = kmpkg::parse_qualified_specifier(
  ...
  on expression: zlib[feature]:x64-uwp
    ...
    SECTION ("parsed qualifier platform expression")
    {
        ...
        REQUIRE(!spec.features);
        REQUIRE(!spec.triplet);
        REQUIRE(to_string(spec.platform.value_or_exit(KMPKG_LINE_INFO).value) == "windows");

        auto forbidden_spec = kmpkg::parse_qualified_specifier(
