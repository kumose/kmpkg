...
kmpkg_download_distfile(ARCHIVE
    URLS "https://kmpkg-download-distfile.dev/${VERSION}.tar.gz"
         "https://kmpkg-download-distfile.dev/${VERSION}-other.tar.gz"
    ...
    auto expected = Json::parse(R"json(
{
  "packages": [
    ...
    },
    {
      ...
      "downloadLocation": "https://kmpkg-download-distfile.dev/3.2.1.tar.gz",
      "licenseConcluded": "NOASSERTION",
      "licenseDeclared": "NOASSERTION",
      "copyrightText": "NOASSERTION",
      "checksums": [
...
TEST_CASE ("spdx maximum serialization", "[spdx]")
{
    ...
    const auto sbom =
        create_spdx_sbom(ipa,
                         std::vector<Path>{"kmpkg.json", "portfile.cmake", "patches/patch1.diff"},
                         std::vector<std::string>{"kmpkg.json-hash", "portfile.cmake-hash", "patch1.diff-hash"},
    ...
    static constexpr StringLiteral expected_text = R"json(
{
  ...
  "creationInfo": {
    "creators": [
      "Tool: kmpkg-2991-12-31-unknownhash"
    ],
    "created": "now"
  },
  "relationships": [
  ...
  "packages": [
    {
      ...
      "comment": "This is the port (recipe) consumed by kmpkg."
    },
    {
      "name": "zlib:arm-uwp",
      ...
      "licenseDeclared": "NOASSERTION",
      "copyrightText": "NOASSERTION",
      "comment": "This is a binary package built by kmpkg."
    }
  ],
  "files": [
    {
    ...
    auto expected = Json::parse(expected_text, "test").value(KMPKG_LINE_INFO);
    auto doc = Json::parse(sbom, "test").value(KMPKG_LINE_INFO);
...
TEST_CASE ("spdx minimum serialization", "[spdx]")
{
    ...
    const auto sbom = create_spdx_sbom(ipa,
                                       std::vector<Path>{"kmpkg.json", "portfile.cmake"},
                                       std::vector<std::string>{"hash-kmpkg.json", "hash-portfile.cmake"},
                                       {},
                                       {},
                                       "now+1",
